...
默认在本机保存和管理资料，无需部署服务。**0.8.0 Beta 新增独立服务端**：把资料库、图片和媒体目录放在 NAS 或常开电脑上，通过桌面应用远程管理，通过浏览器浏览与播放。
...
Beta 发布后可直接拉取 Docker 镜像，无需编译源码，支持 x86_64 和 ARM64。请从 [服务端部署指南](docs/SERVER_MODE.md#docker-compose-快速开始) 开始，了解首次连接、媒体目录和更新备份。该版本仍在验收收尾，建议先用独立测试资料库体验；已有资料升级前请完整备份。预发布包与部署配置以 [GitHub Releases](https://github.com/JavdexLabs/Javdex/releases) 中的 Beta 条目为准，正式版下载入口不会自动切换到 Beta。
...
- 本地模式下，尚未收录的条目可按选项创建为无资源影片，先保存资料，之后再添加资源；远程清单导入复用已有影片。清单导入不会下载影片。
