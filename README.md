...
- 扫描首次发现资源时可一次性导入相邻本地 NFO；没有 NFO 不产生警告，已刮削影片不会被自动覆盖
- 可从“设置 → 存储”先生成不可变计划，再以前台阻塞任务导出通用/Kodi、Jellyfin、Emby、Plex NFO Agent 或 Infuse 格式的 NFO 与本地图片
- 本地保存封面、海报、样张、头像与写真
- 支持单项刮削、批量刮削、重匹配和字段级更新策略
- 批量任务可暂停、恢复或取消

NFO 导入与导出都是一次性快照，不监视 sidecar、不保存 NFO 关联或 manifest。导出默认跳过已有文件，覆盖前必须重新生成并确认计划；详细 profile 边界见 [NFO 导出兼容性](docs/NFO_COMPATIBILITY.md)。
