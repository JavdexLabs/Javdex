...
项目官网由 GitHub Pages 托管，源码位于 [`website/`](website/)。本地构建与预览：
...
Pages 构建会读取最新正式版 GitHub Release，为 Windows、macOS 和 Linux 生成对应安装包下载链接。
首次部署前，需要在仓库 `Settings → Pages` 中将 `Source` 设置为 `GitHub Actions`；后续合并到 `main` 或发布正式 Release 时会自动更新官网。
