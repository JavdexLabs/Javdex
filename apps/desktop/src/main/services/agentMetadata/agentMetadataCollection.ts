...
/** Read mutable product state again after awaited work instead of trusting an earlier narrowing. */
function currentPhase(active: ActiveMetadataRun): AgentMetadataPhase {
  return active.state.phase
}
...
const TARGET_PROMPT_BUILDERS = {
  video: (target: Extract<AgentMetadataTarget, { kind: 'video' }>): string => {
    const video = getVideoById(target.id)
    if (!video) throw new Error('影片不存在。')
    return `当前目标是影片 #${video.id}，库内番号为「${video.code}」。必须在页面中找到并核对同一番号。`
  },
  actress: (target: Extract<AgentMetadataTarget, { kind: 'actress' }>): string => {
...
export class AgentMetadataCollection {
    ...
    return toolHost.registerRun({
      ...
      handlers: createAgentMetadataToolHandlers({
        ...
        submit: async (args, signal, callId) => {
          ...
          if (signal.aborted || currentPhase(active) === 'cancelled') {
            agentMetadataDraftService.discard({
              draftId: draft.id,
              expectedRevision: draft.revision
            })
            signal.throwIfAborted()
            throw new Error('元数据采集已由用户终止。')
          }
  ...
  private releaseExecution(runId: string, reason: string, terminal = false): void {
    const active = this.active.get(runId)
    if (active?.liveEmitTimer) {
      clearTimeout(active.liveEmitTimer)
      active.liveEmitTimer = undefined
    }
    toolHost.disposeRun(runId)
    void Promise.allSettled([
      agentMetadataBrowser.release(runId, reason),
      terminal ? agentExecution.closeRun(runId) : agentExecution.releaseRun(runId)
    ])
  }
  ...
  private project(
    ...
    if (
      ...
      ['collecting', 'preparing', 'waiting_user'].includes(active.state.phase)
    ) {
      active.state.phase = 'failed'
      active.state.summary = event.message
      active.state.errorCode = 'RUNTIME_FAULT'
      changed = true
      this.releaseExecution(runId, 'Metadata Agent runtime fault', true)
    }
    if (event.type === 'limit.reached' && active.state.phase === 'collecting') {
      ...
      changed = true
      this.releaseExecution(runId, 'Metadata Agent turn limit', true)
    }
    if (event.type === 'agent.settled') {
      if (active.state.phase === 'collecting' || active.state.phase === 'preparing') {
        ...
        this.releaseExecution(runId, 'Metadata Agent settled without result', true)
      }
      changed = true
      if (active.state.phase === 'ready') {
        this.releaseExecution(runId, 'Metadata candidate ready')
      }
    }
    if (changed) {
      active.state.revision += 1
  ...
  async start(input: AgentMetadataStartInput): Promise<AgentMetadataSnapshot> {
    ...
    try {
      ...
      await Promise.allSettled([
        agentMetadataBrowser.release(runId, 'Metadata Agent start failed'),
  ...
  async resume(input: AgentMetadataResumeInput): Promise<AgentMetadataSnapshot> {
    ...
    const handoff = structuredClone(active.state.handoff)
    ...
    try {
      ...
      if (active.state.phase === 'collecting' && !active.state.handoff) {
        active.state.phase = 'waiting_user'
        active.state.summary = '继续失败，浏览器仍在等待你的操作'
        active.state.handoff = handoff
        this.persist(input.runId, active, 'metadata.resume-failed')
      }
      throw error
    }
  }
  ...
  async cancel(runId: string): Promise<void> {
    const active = this.active.get(runId)
    if (!active || ['applied', 'routed_to_pending', 'discarded', 'cancelled'].includes(active.state.phase)) return
    const draft = agentMetadataDraftService.findByRunId(runId)
    if (draft?.status === 'ready') {
      agentMetadataDraftService.discard({ draftId: draft.id, expectedRevision: draft.revision })
      active.state.draftId = draft.id
    }
    ...
    toolHost.disposeRun(runId)
    const closed = await Promise.allSettled([agentExecution.closeRun(runId)])
    this.active.delete(runId)
    for (const result of [...cleanup, ...closed]) {
      if (result.status === 'rejected') {
        console.error('Metadata Agent cancellation cleanup failed:', result.reason)
      }
    }
  }
      ...
      try {
        ...
        if (
          ...
          agentMetadataDraftService.discard({
            ...
            expectedRevision: ownedDraft.revision
          })
          active.state.phase = 'failed'
          active.state.draftId = ownedDraft.id
          active.state.summary = '目标已不存在，待确认的元数据草稿已清理。'
          active.state.errorCode = 'TARGET_MISSING'
          agentRunStore.updateProductState(record.id, 'failed', active.state)
          agentRunStore.closeRun(record.id)
          this.active.delete(record.id)
        } else if (ownedDraft?.runId === record.id && ownedDraft.status === 'ready') {
          active.state.phase = 'ready'
          active.state.draftId = ownedDraft.id
          ...
          if (state.phase === 'ready') {
            active.state.phase = 'failed'
            active.state.summary = '待确认的元数据草稿已丢失，请重新发起采集。'
            active.state.errorCode = 'DRAFT_MISSING'
            agentRunStore.updateProductState(record.id, 'failed', active.state)
          }
          agentRunStore.closeRun(record.id)
          this.active.delete(record.id)
  ...
  async dispose(): Promise<void> {
    ...
    for (const [runId, active] of this.active) {
      ...
      toolHost.disposeRun(runId)
      releases.push(agentExecution.releaseRun(runId))
    }
  ...
  private retireDraftRun(draftId: string): void {
    const runId = agentMetadataDraftService.getDraft(draftId)?.runId
    if (!runId) return
    const active = this.active.get(runId)
    if (active?.liveEmitTimer) clearTimeout(active.liveEmitTimer)
    this.active.delete(runId)
    this.releaseExecution(runId, 'Metadata draft reached a terminal state', true)
  }
