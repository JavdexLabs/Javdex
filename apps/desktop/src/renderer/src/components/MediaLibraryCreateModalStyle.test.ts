...
function declarationsFor(selector: string): Map<string, string> {
  const source = readFileSync(
    path.resolve('apps/desktop/src/renderer/src/components/MediaLibraryCreateModal.module.css'),
    'utf8'
  )
...
describe('MediaLibraryCreateModal control style', () => {
  it('keeps all workflow steps at one viewport-safe dialog height', () => {
    ...
    const component = readFileSync(
      path.resolve('apps/desktop/src/renderer/src/components/MediaLibraryCreateModal.tsx'),
      'utf8'
    )
