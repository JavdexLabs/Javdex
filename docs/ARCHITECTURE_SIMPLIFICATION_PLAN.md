...
状态：实施中，P1–P3 已落地并完成定向验证，P4 已开始，P0 跨存储核对仍在推进；P4/P5 未完成。日期：2026-09-20。重构前代码基线：`dd13789`。
...
尚未完成的 P4 内容：草稿仓储显式接线、catalog 回执与工作状态间的中断恢复、删除/图片清理等交叉调用改造、移除运行时 ATTACH/SQL 改写。上述连接改造仅覆盖运行存储，不表示全部工作表已脱离隐式路由。

未完成：P0 的跨存储提交失败特征测试、P4 的上述剩余实现、P5 全量集成与平台 GUI/升级验证。既有平台验收不算本次重构验收。
