...
> 后续范围决定（2026-09-20）：用户另行批准取消历史冷恢复和精简模型配置，见 [ADR-0030](adr/0030-simplify-agent-recovery-and-model-configuration.md)。下文“保留冷恢复”及第 10 节对应待定项记录的是本方案当时范围；清单导入继续保持现状。

状态：已完成，P0–P5 逐项验收通过，完成审计见第 12 节。日期：2026-09-20。重构前代码基线：`dd13789`；最终生产修复：`de1b0bc`，安装验收脚本及实现文档：`e29db2a`。仅本地提交，未推送或发布。
...
当前代码和各阶段验证见第 11 节；其中实施过程记录按时间保留，后续条目取代前面的临时状态。第 12 节是最终完成结论，不再保留平行 SQL 路由实现。
...
审计以当前源码、测试断言及实际命令结果为依据。第 11 节的“未完成/尚待”是各阶段当时状态，以下结果覆盖这些遗留项。生产变更止于 `de1b0bc`，之后只新增安装验收脚本与文档。核对中发现的丢弃意图中断缺口已在该提交修复，未以旧产物替代最终修复的启动验收。

| 要求 | 当前实现与直接证据 | 判定 |
|---|---|---|
| P0、A1：详情共同投影、宿主路径策略 | 两端实际调用 `projectVideoDetail`；`videoQueryService.test.ts` 覆盖资源、STRM、外链脱敏、时长优先级、无归属及无资源；Node runtime 经真实 HTTP 对照本地投影 | 通过 |
| A1：播放不能执行展示路径 | 资源 ID/定位摘要及播放授权路径保留；runtime 实际 Range/mpv 测试及 Linux 安装 GUI 播放通过 | 通过 |
| A2：演员编辑编排唯一、权限留宿主 | 本地 `localCatalogBackend` 和 Node `manageCatalogHandlers` 均调用 `editCatalogActress`；HTTP 保留 writer 校验，本地旧无 A 入口仅显式开启兼容 | 通过 |
| A2：版本、头像、冲突、失败及回执 | `catalogActressEdit.test.ts` 的 6 项覆盖单次 revision、重放、陈旧版本、清除/替换头像、冲突及失败恢复；runtime 验证真实上传与 HTTP 编辑/清除 | 通过 |
| A3：正式清单写入共同所有者 | 桌面 Repository 与 `catalogPlaylistImport` 均调用 `writePlaylistImport`；原平行正式 SQL 编排已删除 | 通过 |
| A3：本地追加/建片与远程限制保留 | 共享写入 3 项覆盖排序去重、追加、新片、相关链接及全部回滚；Repository 与管理测试覆盖过期匹配、重复提交；宿主固定归属策略，HTTP 不允许自报能力 | 通过 |
| B1：业务字段权威定义 | 详情、演员编辑、清单结果由 contracts schema/推导类型定义；旧导出仅为类型兼容，不留第二份字段表；IPC 与 HTTP 仍各自处理路径和凭据 | 通过 |
| B2：实际 JSON 校验及静态宿主结果 | `catalogRemoteResults` 为选定用例的 11 个复杂结果入口执行 schema 解析；用例返回权威类型，根/服务端类型检查通过；其余协议未扩大重写 | 通过 |
| B2：真实往返、错误类型/缺字段/泄漏拒绝 | runtime 的影片、演员、清单往返测试直接使用 `RemoteCatalogBackend`；影片代理负例删除字段、改类型、注入原始路径；分页筛选/排序与日期/null 保留有断言 | 通过 |
| C1：表、调用者、TEMP、图片归属 | 第 11 节归属清单已核对；清单 TEMP 与本地正式写入继续在其原事务中，Node 草稿仍显式使用 catalog；桌面运行/草稿转入 workStore | 通过 |
| C2：显式连接、未配置拒绝及退出解绑 | `AgentRunStore`、`AgentMetadataDraftRepo` 必须提供连接；composition root 绑定/解绑；实际启动和显式连接测试覆盖未配置访问和重开 | 通过 |
| C3：一次性复制、源数据、内容与外键 | `agentWorkCopy.test.ts` 4 项验证 ID/关系/可解密内容、等行数内容损坏、孤儿记录回滚与 ready 拒绝覆盖；源表未删除，历史 migrations 未改写 | 通过 |
| C3：中断、重试、ready 后不覆盖 | isolation 测试在复制过程及开放 SQL 事务内 SIGKILL，确认源保留与 copying 状态；启动测试和两平台最终安装测试验证复制及重启后新工作记录保留 | 通过 |
| C3：删除正常运行 SQL 路由 | 源码已无 `attachAgentWorkStore`、`wrapCatalogSqlForAgentPrefix`、`qualifyAgentSql`、`agentWorkTablePrefix`；真实启动断言无 work 附库和数据库方法覆盖；唯一复制 ATTACH 随流程 DETACH | 通过 |
| C4：应用持久意图→catalog 回执→工作完成→清理 | `draftApplyCommit` 及独立 catalog/work 服务测试验证提交前失败、正式提交后工作失败、清理失败、真实关闭重开、幂等恢复、版本不重增 | 通过 |
| C4：未知状态与不可变提交 | 同 catalog/预览/请求摘要检查保留；恢复先读回执，再验证资源；无回执不标成功，变化请求拒绝，已确认提交不释放；远程断线及回执重连由既有后端/runtime 测试验证 | 通过 |
| C4：丢弃中断及重启恢复 | 管理后端测试覆盖工作状态失败、原请求重放和清理；新增真实 workStore 重开测试验证提交前退出后安全释放并允许新请求；协调器最终 9 项通过 | 通过 |
| 跨存储：删除和暂存资源清理 | 草稿仓储生命周期快照测试覆盖终态/资源身份、变化拒绝和重复清理；启动测试覆盖删除后工作失败、异 catalog 回执拒绝及重开清理 | 通过 |
| 跨存储：图片引用与导出隔离 | `catalogExportWorkIsolation.test.ts` 使用源 catalog、独立导出库及 live workStore，断言只清理导出副本；暂存引用按显式仓储和目标类型读取 | 通过 |
| 隔离与现有 Agent 能力 | 启动/隔离最终 23 项通过，包含无法读取本机 catalog 的远程启动与功能入口；扩大回归覆盖日志/审批、加密恢复、检查点及冷恢复，不削减现有能力 | 通过 |
| P5：类型、边界、lint | 根 typecheck 通过；最终 `npm run pretest` 退出 0，含 workspace/边界、TS/CSS lint 和 UI 控件检查，日志 `/tmp/javdex-p5-final-pretest.log` | 通过 |
| P5：受影响 Electron 回归 | 初次 879 项中 877 项通过；两项测试平台/结构化错误断言修正后定向通过，Linux ENOSPC 有权限环境补跑通过；最后本地恢复改动另有 9+23 项通过。没有把跳过或最初失败记录计作通过 | 通过 |
| P5：Node 与 Docker | `server:test` 46+5+5 项通过；2 项原生窗口转 Xvfb 实跑通过；Node build、普通 Docker、rshared 双宿主迁库（含 EACCES/ENOSPC）均通过。生产 Node 代码此后未变 | 通过 |
| P5：构建、GUI、安装升级 | 最终桌面/Web 构建、DMG/deb 构建和同版本安装烟测通过；两平台远程业务 GUI/替换安装通过；最终修复版 DMG 副本和正式 deb 安装额外通过本地迁移及重启不覆盖 | 通过 |
| 文档、ADR、范围与交付 | 开发指南及合同清单已同步；未改正式资料同库回执语义，无需改写旧 ADR；第 10 节其他建议仍待定；无版本升级、发布、推送、合并或通用框架 | 通过 |

量化结果：三组业务路径共享实现，11 个复杂结果入口运行时校验，4 项复制专门测试及 2 种实际 SIGKILL 中断场景；正常运行 SQL 改写入口为 0，一次性迁移入口保留。新增持久恢复代码用于维持独立存储下原有能力，不把净删行数作为成功标准。

平台结论仅限已约定的 macOS arm64、Linux arm64；Windows/macOS x64 仍是后续事项。测试夹具与日志是本次本机验收证据，持久可复跑入口为仓库测试和升级烟测脚本。本方案所有纳入范围的事项已完成；未创建外部工单或 PR，未动独立许可证研究文件。
