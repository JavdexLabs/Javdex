...
勾选「记住此设备」可跨应用重启保持登录，仍受闲置 24 小时与原始授权起最长 7 天的期限限制。临时登录不跨应用重启保留。桌面刷新状态后可查看设备名称、最近访问时间并逐台撤销。设备名称由浏览器提供，仅用于辅助核对。
...
- 桌面撤销设备会关闭当前所有 Web 连接，其他未撤销设备可重新连接。无法收回已下载的视频或浏览器缓存。
- 配对请求限时 5 分钟，批准后最多 60 秒内兑换且不延长原期限。六位码只用于桌面核对，兑换另需绑定浏览器的随机秘密，且只可成功兑换一次。拒绝、取消、过期、服务重启或撤销全部授权均使待配对请求失效。只有桌面能批准，普通只读浏览器没有授权其他设备的能力。
