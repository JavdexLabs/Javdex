...
- 提供“导出样张”开关，默认关闭。
- 开启后写入资源目录下的 `extrafanart/`，文件名为 `<videoStem>-001.<ext>`、`<videoStem>-002.<ext>`，按 Javdex 当前样张顺序稳定编号。
- stem 前缀保证多个影片共享同一目录时互不碰撞。
- 目标 profile 支持本地样张引用时，在 NFO 中写入相对引用；目标软件不支持时仍可复制样张，但计划必须产生兼容性提示。
- 预览单独展示样张文件数与预计写入体积。
...
3. 用影片字段、关系、站点统计、资产路径/内容 hash、资源锚点及媒体库 revision 生成 source snapshot hash。
4. profile renderer 生成稳定 XML；资产项通过 `MediaAssetStore.readBytes` 读取，计划记录源 hash、目标 realpath、existing hash/mtime、action 与预计字节数。
5. 检测同一目录内不同全局影片的同番号 warning、目标路径碰撞、无 fanart、无头像、profile 不支持样张引用及不能表示的字段。
...
- `apply` 逐项复核 source snapshot、目标 realpath 与 existing hash/mtime；变化项报告 `stale-plan`。
...
- 没有 NFO 的首次发现只进行有界 sibling 查找，不解析目录中无关 XML；既有资源不触发 NFO I/O。
