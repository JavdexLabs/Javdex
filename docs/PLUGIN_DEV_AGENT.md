...
PluginDeveloper 采用“Pi 主导、宿主轻量”的自适应开发架构。Pi 与冻结的 Skills 负责浏览、外部开发笔记、字段范围、代码实现和结果语义判断；Javdex 只提供隔离工作区、浏览器、生产沙箱、最新运行事实和安装机械门禁。简单网站在一个连贯开发过程中按搜索页、详情页固化代码，再执行第一次完整验证；只有真实运行缺口才进入多轮迭代。

## 职责边界

开发助手的插件选择列表遵循插件描述中的 `debuggable` 能力。Gfriends 是头像索引集成，不支持 AI 调试，因此不出现在选择列表中；插件卡片中的 AI 调试入口也禁用。其正常刮削能力不受此限制影响。

Pi 负责：

- 找到代表性的精确详情页，并从页面提取真正的影片番号或演员主名/别名；
- 根据页面事实判断字段含义、选择器和返回值是否正确；
- 在离开搜索页前固化生产搜索逻辑，再在详情页固化全部已明确字段，并维护 `.javdex/dev-notes.md` 与 `plugin.json.supportedFields`；
- 比较 `pluginResult`、`effectiveResult` 和 `manifestCoverage`；
- 搜索与详情实现完成后执行第一次完整 dry-run；只有明确错误或未完成字段存在时才使用 `write/edit ↔ plugin_dry_run` 继续迭代。
...
create 模式采用两个不运行的页面级代码检查点：

1. **搜索页代码检查点**：先按 `javdex-plugin-dev` 的四档顺序确认搜索入口。已有可提交的搜索控件或可 `open` 的搜索链接时，必须先 `fill` / `press` / `click` 或直接 `open`；`action` 为空或控件无 `name` 不是跳过第一档的理由。提交后 observation 的 `url` 变为另一个地址才算第一档成功；同一 `url` 上的 overlay 或 AJAX 结果继续降脚本、`recentRequests`、生产 `ctx.browser`。在进入详情页前必须收齐搜索 URL 或请求构造、候选容器或响应集合、详情/资料链接选择器或字段、候选身份信号和相对 href 基准。证据齐全后立即把生产搜索定位函数写入 `index.js`，同步搜索事实到 dev-notes；代码只要求语法有效，暂时不要求插件完整，也不调用 `plugin_dry_run`。为还原请求而打开站内脚本仍属于搜索阶段，但不得先打开详情页、以后再回来补搜索选择器。
2. **详情页代码检查点**：搜索逻辑写入后，只打开一条精确详情页或资料页。收齐当前页面已明确的身份、字段、选择器与 metadata 后，立即实现详情解析并更新 dev-notes。没有新事实、返回 `unchanged` 或只剩理论问题时停止浏览，不能按字段逐项证明。第一次完整 dry-run 前不得返回搜索页；只有 dry-run 暴露与搜索候选相关的具体错误，才针对该 blocker 回访。

首次完整运行前，不为验证理论分支而另外浏览空结果页、推荐结果、模糊搜索、分页或镜像域名。空结果和多候选合同先按 `docs/plugin-format.md` 实现；搜索与详情代码、manifest 和 dev-notes 同步后再执行第一次完整生产 dry-run。页面级检查点按页面划分，不是按字段拆分；简单网站仍应在一个连贯开发过程内完成当前页面全部已观察且映射明确的字段。影片搜索页若有多条番号完全匹配，浏览不要把其余候选点开；生成的影片插件仍须抓取第一页全部完全匹配详情，多于一条返回对象数组，任一条详情失败则整次失败。`runTargets` 为空时浏览发现身份；用户目标零条精确匹配时不检查推荐结果结构，另外找到代表性精确详情，按既有空完整 dry-run 与 `ask_user` 流程处理。
...
检查点损坏或不兼容时，主进程明确拒绝续跑并提示新建任务，不再从数据库历史冷重建。已有工作日志、审批和工具记录保留；恢复失败的工作区不会因离开页面自动删除，显式“清除会话”仍可清理。新任务的压缩预算与缓存保留由内部策略决定，已有任务继续使用冻结设置；见 [ADR-0030](adr/0030-simplify-agent-recovery-and-model-configuration.md)。
...
浏览器工具保留 64 KB 页面 observation 硬上限。完整 observation 始终按内容寻址写入 browser artifact v1。超限时按完整 section 装包：`snapshot` 与每个 `pageFacts` section 都不可切开，从最大的整段开始省略，直到落入硬上限；被省略的 section 只报告 `itemCount` / `byteLength`，`nextActions` 为 `find` / `html` / `read-section`。宿主不按页面类型或字段相关性挑选 section，也不截取数组前 N 项。`omittedInlineSections` 中的每个名称都能从同一 artifact 读取；Pi 只需用 observation 返回的 `artifactRef` 和 section 名调用 `browser(action="read-section", ...)`，接口返回有界页面和不透明 `nextCursor`，不获取或占用浏览器租约。请求不存在的 section 时返回 `validSections`，并通过 `nextAction` 提示先读取 `section="observation"` 的目录。artifact 的索引、分片、完整性校验和路径约束由 `PluginBrowserCapabilityModule` 隐藏，不再要求 Pi 理解或原生读取存储格式。`artifactComplete` 与 `inlineComplete` 分别描述 artifact 和 Agent 当前内联结果。
...
- 首次完整 dry-run 前不主动浏览空结果、推荐、模糊搜索、分页或镜像分支；生成的影片插件仍须抓取第一页全部番号完全匹配，多于一条返回数组，任一条详情失败则整次失败。
