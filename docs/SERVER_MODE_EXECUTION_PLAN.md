...
| S13 | 进行中：本轮补上 M08 页间真实 umount、M06 扫描中领取 `waitingMaintenance`、M09 过期/重启 NFO 计划、D04 用户态代理 TCP RST。不得宣称 M01–M15 / D01–D07 全部完成 | 见本文件 S13 实施记录 |
...
- M05：`catalogOperations` 在 SQLite 事务内、mutate+回执 INSERT 之后、函数返回之前挂起；测试子进程 `SIGKILL` 后标题/revision 不变、无回执，同 `operationId` 重试才应用一次（`bebde60`）。这是 library 子进程被杀，不是整台 HTTP 宿主断电。
...
- 验证（Linux Node 22.14 / amd64 glibc；另见 `3c9f97b` / `5589d42` / `83d6cf2` / `65c3e1c` / `5ebff2d` / `4822fb6` / `1bac323` / `4c79d67` / `e187266` / `6ad4b84` / `156c1a8` / `5699eae` / `8d3d8f4` / `544e099` / `747ab75` / `933797f` / `e9ee433` / `5401689` / `3152bf1` / `86a82b5` / `870ddf2` / `bc019ee` / `2dd1e54` / `8fc3d9a` / `cb13f72` / `6be92a7`）：
  ...
  - 本会话 `bc-59bde151-be0b-4c9e-8ca2-12376b967412`：`tsc --noEmit` `tsconfig.server.json` / `tsconfig.node.json` 通过。先前 `runtime.test.ts` **21 通过**（S09 / 首个 M08 umount / M15 mpv）。本轮续：`catalogOperations.crash.test.ts` **1 通过**（M05 提交前 SIGKILL）。定向 `runtime.test.ts` M06 in-flight claim、M06 扫描 `waitingMaintenance`、M08 页间真实 umount、M09 过期/重启 NFO、D04 TCP RST **5 通过**。全量 `apps/server/src/runtime.test.ts` **26 通过 / 0 失败**（含 M15 `/usr/bin/mpv`）。本机有 `sudo mount --bind` / `umount` 与 `/usr/bin/mpv`；无 `docker`，未重跑容器烟测或全量 Electron。
...
- 未做：Windows NSIS/ZIP 与 macOS DMG 安装包烟测（本 Linux VM 不能诚实构建/签署）；远程 FILE_RENAME（C1）；D04 WAN 丢包后再重连；D05 任务进行中的完整 GUI 关窗产品流；M07 三目标分页 UI 拼接；M05 整台 HTTP 宿主断电；M15 非环回 WAN；完整 M01–M15 / D01–D07。不得把 mock 当完成证据。不得宣称 S13/S14 或 M/D 矩阵已全部完成。
...
| M05 受理边界 | 部分 | S06 上传/apply/重启；S09 任务入队；`bebde60` 在打开的 SQLite 事务内 SIGKILL library 子进程，短 `videos.edit` 回滚 | Linux Node 22.14；独立 child | 不是整台 HTTP 宿主/断电/fsync 失败 |
