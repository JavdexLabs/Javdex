...
| S13 | 进行中：本轮补上 M05 HTTP 宿主提交前 SIGKILL、M12 STRM 规范化冲突的两端 HTTP、M15 非环回听地址 mpv、D03 复制 SQL 事务中途 SIGKILL。不得宣称 M01–M15 / D01–D07 全部完成 | 见本文件 S13 实施记录 |
...
- 本轮新增 `scripts/server-migration-smoke.mjs` / `npm run server:smoke:migration` **EXIT 0**（`bc-d991d9d2-a4ef-57ac-86c9-f10d2c228a95`；Engine 28.5.2 / fuse-overlayfs）：两个 `javdex-server:smoke` 容器，源与空目标各用独立 `dataDir` 与 `imagesDir` 卷；源夹具按 `migrationHosts.e2e.test.ts` 写入番号 + 正式封面；两端 `docker exec migrate-auth`；宿主经 manage HTTP 走 preview → source start → PUT 包 → target start → enable。断言两端 `migration.status`（源 `frozen` / 目标 `enabled`）、目标正式图文件、源封面仍在（冻结备份）、目标新 `catalogId`。本会话续：`4e5c74d` 在同一包再导入第三生产容器后并发 `enable`/`abandon`，终态互斥（enabled 迟到 abandon 仍 enabled；abandoned 迟到 enable 401 `AUTH_REQUIRED`）。`efabe5e` 另把同一 400KiB 正式封面导入第四/第五生产容器：宿主 `chmod 000` 图片绑定卷后 `migration.enable` HTTP 400 且报文含 `EACCES`，`targetPhase` 仍 `ready`、目标空库、正式封面不存在，恢复 0755 后重试 enable 成功并写出封面；`sudo mount -t tmpfs -o size=256k,mode=1777` 后 enable HTTP 400 且报文含 `ENOSPC`，同样回滚 `ready`，`docker stop` 后卸载 tmpfs，同一 `dataDir` 再拉起后重试 enable 成功。
...
- M12：启用后对目标 `library.db` 做真实 `scanCoordinator.run`。未映射孤儿片无本地资源、自动清理关闭时成员保留；把 `remove_resource_less_memberships` 改回 1 再扫才会删成员（`e05c165`）。本轮两个 OS 进程：源上两条未映射 STRM 转同一 `direct` locator，`migration.preview` 带 `strmConflicts`，`migration.start` HTTP 400 `INVALID_INPUT`「迁移预检未通过」，源未冻结、无包文件，空目标 `videos` 仍为 0 且 `migration.status` 为迁移不存在（`b3de99b`）。映射根上的 STRM：preview `strmConversions=0` 且无冲突，enable 后目标 `strm_source_path` 与本地 locator 落在目标挂载，URL locator 不变，源路径仍为原 STRM 文件（`292486c`）。
...
- D03：workStore 已复制行但 `prepStatus=copying` 时远程仍 `modePrepRequired` 且不 `getDb()`；回到本地才 `markReady`，源 `agent_runs` 仍在。OS 级 `SIGKILL`：子进程 `beginCopy` 后、`copyAgentWorkTables` 前被杀，`prepStatus` 保持 `copying`、目标 `agent_runs` 为 0、源行仍在；远程仍 `modePrepRequired`，回本地后复制完成（`933797f`）。本轮另在 `INSERT OR REPLACE` 事务内、commit 前挂起再 `SIGKILL`：dest `agent_runs` 仍 0（未提交复制回滚），源行仍在，远程仍 `modePrepRequired`，回本地才 `markReady`（`d8b5429`）。
...
- M15：真实 `/usr/bin/mpv` 0.37.0 + ffmpeg `testsrc` H.264/AAC 播放 Range grant：`closePlayStreams` 后同一 handle 再 `loadfile` 可续播；把 `play-grant` `expiresAt` 写成过去后 `HEAD 404` 且 mpv 日志含 HTTP 404；`seek 2.2 absolute` 落到 ≥2.05；MKV 同样解码；writer handoff 后 seek/`HEAD` 404（`e9ee433`）。本轮另把 manage/play 绑到非环回地址 `10.67.67.1`（`sudo ip addr add` 到 `eth0`；内核无 dummy 模块，`ip link add type dummy` 为 `Unknown device type`）：`play.grant` 的 `playbackHandle` 含该主机，`HEAD 200`，mpv `loadfile` 解码到 `time-pos` ≥0.2（`b3940d0`）。这是本机非环回别名，不是 WAN。
...
- D04：`waitForCatalogTask` 按 generation / catalogId / taskId / taskRevision / progressSeq 应用 `tasks.get` 快照，终态不回绕；重连 generation 丢弃旧成功快照；失败退避至 30s；`SCAN_CANCEL` / `NFO_EXPORT_TERMINATE` 中止本地等待（`870ddf2`）。`8fc3d9a` 对真实 Node 宿主 `tasks.get`：HTTP 响应已返回后 `reconnect()` 推进 generation，再把该快照交给 `waitForCatalogTask`，旧 generation 不应用，后续 poll 才应用终态。`cb13f72` 另在真实 `tasks.get` 已受理后卡住响应，再 `reconnect()` 中止 in-flight fetch：3s 内完成，不等待 8s 服务端 stall。本轮：真实 HTTP `videos.get` 在响应返回后 `reconnect()`，迟到详情不作为当前行；`readImage` 在 reconnect 后中止，迟到 PNG 不写入 catalog 磁盘缓存，后续 GET 重新取数。`a33303c` 另用用户态 HTTP 反代把 `Host`/`Origin` 改写成服务器公布地址，丢掉已收到的 `videos.get` 响应字节后对客户端 socket `resetAndDestroy()`：in-flight 查询得 `CONNECTION_UNAVAILABLE`（`无法连接远程资料库`，不是 AbortError），`reconnect()` 推进 generation 后同一 `videos.get` 成功。这是环回 TCP RST，不是 WAN 丢包。
- D05：`createWindow` 不注册 IPC；远程 runtime dispose 后再装配仍不打开 `library.db`。主窗口重建调用已注册的 `bindMainWindow` binders，不二次注册 IPC（`870ddf2`）。`6be92a7` 用原生 Electron 关闭后再建 `BrowserWindow`：新 `webContents` 只绑定一次。本轮：已受理 `scans.run` 后中止 `waitForCatalogTask`，dispose 远程后端再新建连接，`operations.get` 仍能按原 `operationId` 读到 `acceptedTask`/`applied`。`e674dac` 另在真实 HTTP 宿主把扫描停在 `afterEnumerate` 时关闭原生 `BrowserWindow` 再创建：in-flight 等待中止，`operations.get` 仍是原号，放开 stall 后任务终态 succeeded/needsInspection。不是渲染器扫描按钮/工具栏产品流。
- M01：同一 Node 宿主用例一次铺齐隐藏成员、归档库、无成员影片、独立演员、只含隐藏片的清单、待确认刮削图。管理 Bearer 可读；网页 Cookie 对隐藏/无成员/独立演员/归档库/待确认图均为不可见或 401；Cookie 不能调 `videos.get`。
...
- M07：三个独立 `videos.edit`：过期 V 得 409 且标题不变；成功项同 `operationId` 重试回执为 `duplicate`；第三项在 commit 后、HTTP 返回前中止客户端，`operations.get` 为 `applied`，同号重试不改第二次标题。`9a90997` 另用真实 manage HTTP 冻结 `targetLists` 后以 `limit=1` 铺三页窗口：删除中间冻结 id 后 live `videos.list` 只剩 2 条，窗口仍是 ready/missing/ready，不把下一条拼进空位。未把 `targetListId` 接入刮削批次 start。
- M05：`catalogOperations` 在 SQLite 事务内、mutate+回执 INSERT 之后、函数返回之前挂起；测试子进程 `SIGKILL` 后标题/revision 不变、无回执，同 `operationId` 重试才应用一次（`bebde60`）。本轮另拉起独立 Node HTTP 宿主，把同一 stall 放进子进程环境，对 `videos.edit` 在 `.ready` 后 `SIGKILL` 该宿主 PID：标题/revision 仍为 original/1、无回执；同 `dataDir` 再启动后 `operations.get` 为 `unknown`，同 `operationId` 重试才改成 `after-kill` / revision 2（`01266e4` / `6e5fe1c`）。`9a90997` 再用 `LD_PRELOAD` 拦截 `library.db*` 的 `pwrite64`/`write`/`fsync` 返回 `EIO`（本 SQLite WAL 构建不会在每次 INSERT 上 fsync）：`videos.edit` 得 `SQLITE_IOERR_WRITE`，标题/revision 仍 original/1、无回执，重启后同号才写成 `after-fsync`。这不是整机断电。
- M06：`videos.edit` 在 mutate 前暂停，此时 `writer.handoffBegin`+claim 消费成功，恢复后旧请求 409 `WRITER_REVOKED`、`operations.get` 为 `unknown`，新 writer 可改标题（`dcd12fb`）。扫描枚举后暂停时 claim 为 `waitingMaintenance`；当前扫描结束后新 `scans.run` 得 409 且报文为「交接等待期间不能开始新的维护」；再 claim 同一 candidate 才 `consumed`，随后扫描成功。旧 secret 为 401 `AUTH_REQUIRED`，新 secret 配旧 epoch 为 409 `WRITER_REVOKED`（`790c12a` / `a33303c`）。
- M08：扫描在枚举后、每个协作清理页前、以及原子清理前重新 `inspectRoot`；已卸载的根从 `safeRoots` 拿掉，不会把挂载消失当成每个文件 `missing` 而删记录。`JAVDEX_TEST_UMOUNT_SCAN` 只在测试指令文件存在时暂停，让测试去 `umount` 真实 `mount --bind`（`522246c`）。`runtime.test.ts`：挂载点卸载后仍存在（区别于 S09 `renameSync` 路径消失和只删标记）；KEEP/GONE 记录保留；原 `.javdex-root` inode/mtime 在 remount 后不变（不自动补建）；再挂上后扫描才清理真正删掉的 GONE 文件；卸载后用挂载时算的 `files.rename` digest 得 409 `VERSION_CONFLICT`。页与页之间：`PG-001`…`PG-129`（`parseCode` 拒绝 `PG-000`）导入后删文件，第一页提交 128 条 missing-resource 删除，第二页前真实 `umount`，剩 1 条；remount 后原标记 inode 不变，再扫清掉最后一条（`dcd12fb` / `790c12a`）。
- M09：`nfo.plan` 后把 `expires_at` 写成过去再 `nfo.start` 得 409 `VERSION_CONFLICT` 且不写 XML；未过期计划在 `stop`+同 `dataDir` 再启动后被 `recoverCatalogMaintenance` 丢弃，旧 `planId` 同样 409；新计划可导出 `M09-001.nfo`。随后 `browser.setEnabled false`：handshake `browserEnabled=false`、网页 login 404，管理 `videos.list` 仍 200（`790c12a`）。
- 验证（Linux Node 22.14 / amd64 glibc；另见 `3c9f97b` / `5589d42` / `83d6cf2` / `65c3e1c` / `5ebff2d` / `4822fb6` / `1bac323` / `4c79d67` / `e187266` / `6ad4b84` / `156c1a8` / `5699eae` / `8d3d8f4` / `544e099` / `747ab75` / `933797f` / `e9ee433` / `5401689` / `3152bf1` / `86a82b5` / `870ddf2` / `bc019ee` / `2dd1e54` / `8fc3d9a` / `cb13f72` / `6be92a7`）：
  ...
  - `npm run server:smoke:migration` **EXIT 0**（本会话 `bc-59bde151-be0b-4c9e-8ca2-12376b967412`；Docker Engine 28.5.2，fuse-overlayfs）：`PASS: Docker dual-host migrate-auth, package, start/enable, status phases, official images, source frozen backup, enable/abandon race, copy EACCES/ENOSPC rollback`。覆盖源/空目标、第三容器 enable/abandon 竞态、第四容器 `chmod 000` 拷图 `EACCES` 回滚后重试、第五容器 256k tmpfs `ENOSPC` 回滚后卸载再重试。不是安装包；不是完整 M/D 矩阵。
  ...
  - 本会话 `bc-59bde151-be0b-4c9e-8ca2-12376b967412`：`tsc --noEmit` `tsconfig.server.json` / `tsconfig.web.json` 通过。`dualBackendScan.e2e.test.ts` **5 通过**（含 HTTP 宿主 `videos.edit` SIGKILL、原生 `BrowserWindow` 在远程扫描 stall 期间关闭再创建、catalog `pwrite64`/`fsync` `EIO` 回滚，以及冻结目标列表三页窗口在删除中间 id 后保持 missing 占位）。定向 Electron：`resolveFrozenTargetSlots` / `useFrozenTargetWindow` / `FrozenTargetSlotCell` **4 通过**。`migrationHosts.e2e.test.ts` **5 通过**。`sg docker npm run server:smoke:migration` **EXIT 0**（含拷图 `EACCES`/`ENOSPC` 回滚）。未重跑全量 Electron 或 Windows/macOS 安装包。
...
- 未做：Windows NSIS/ZIP 与 macOS DMG 安装包烟测（本 Linux VM 不能诚实构建/签署）；远程 FILE_RENAME（C1）；D04 WAN 丢包后再重连；D05 渲染器扫描按钮/工具栏关窗产品流；M07 刮削批次 start 仍未走冻结 `targetListId`（需产品决定筛选载荷/批次入口）；M05 整机断电；M15 WAN 丢包；完整 M01–M15 / D01–D07。不得把 mock 当完成证据。不得宣称 S13/S14 或 M/D 矩阵已全部完成。

S13 验收矩阵（核心项；“部分”表示有真实证据但未覆盖该编号的全部安排）：

| 编号 | 状态 | 证据 | 环境 | 剩余 |
|---|---|---|---|---|
| M01 管理范围 | 部分 | S08 + 本轮同一 `runtime.test.ts` 用例：隐藏/归档/无成员/独立演员/隐藏清单/待确认图；管理可读，网页与 Cookie 不可 | Linux Node 22.14 in-process 宿主 | 未再铺独立网页演员详情页或 GUI 对照 |
| M02 字段边界 | 部分 | S08 拒绝桌面路径、未知字段、外库 sample id；本轮 `files.importManual` `../` 逃逸 400 且不泄宿主路径 | 同上 | 其他相对路径别名/符号链接逃逸未再单列 |
...
| M04 关联竞争 | 部分 | S09 删除预览/计划过期；S10 预览后加清单或成员得 409；本轮预览后只加 web 资源、成员数不变，旧 digest 仍 409 | 同上 | 预览后只改 sample 资产集合的独立注入未再单列 |
| M05 受理边界 | 部分 | S06 上传/apply/重启；S09 任务入队；`bebde60` 在打开的 SQLite 事务内 SIGKILL library 子进程；`01266e4` 对独立 Node HTTP 宿主同样在 commit 前 SIGKILL；`9a90997` 对同一 HTTP 宿主在 `videos.edit` 期间让 `library.db*` 的 `pwrite64`/`fsync` 返回 `EIO`，得 `SQLITE_IOERR_WRITE`，重启后同号才应用 | Linux Node 22.14；独立 HTTP 宿主进程；`LD_PRELOAD` | 不是整机断电 |
| M06 接管竞争 | 部分 | S05/S09 交接等待挡住新扫描；`dcd12fb` 暂停中的 `videos.edit` 被新 writer 领取后 `WRITER_REVOKED`；`790c12a` 扫描运行中 claim=`waitingMaintenance`，扫完后新扫描仍 `MAINTENANCE_BUSY` | Linux Node 22.14 in-process 宿主 `runtime.test.ts` | 不是 WAN 上的旧请求 |
| M07 批量部分结果 | 部分 | S10 目标列表冻结；三独立 `videos.edit`：过期 409、成功 duplicate、commit 后断线查回执再同号重试；本轮真实 HTTP 冻结三页窗口，删除中间冻结 id 后 live list 压缩、窗口保留 missing 占位 | Linux Node 22.14 HTTP 宿主 `dualBackendScan.e2e.test.ts`；renderer `useFrozenTargetWindow` | 刮削批次 start 仍未走 `targetListId`；断线是 AbortSignal 不是 WAN |
| M08 真挂载卸载 | 部分 | S09 标记 vs `renameSync` 路径消失；真实 `mount --bind` + `umount`：枚举后、首个 resources 页前、以及 128 条已提交后第二页前卸载；挂载点仍在、记录保留、remount 不补建标记；页间 umount 后剩 1 条，再挂上才清掉 | Linux Node 22.14 in-process 宿主；`sudo mount --bind` | 不是 NAS 拔盘 / Windows 盘符 |
| M09 预览与停服 | 部分 | S09 NFO 计划与关浏览；本轮过期/`recoverCatalogMaintenance` 后旧 `nfo.start` 409，新计划写出 XML；关浏览后管理 `videos.list` 仍 200 | Linux Node 22.14 in-process 宿主 `runtime.test.ts` | 不是 GUI 预览页 |
...
| M11 分页与目标清单 | 部分 | S10 `targetLists`；limit=1 取第一页后插入新片或删除已冻结 id，offset=1 与完整 page 仍是冻结快照；本轮 `FrozenTargetSlotCell` 对已删除冻结 id 显示「已删除」占位，三页窗口不压缩 | Node 宿主 `runtime.test.ts`；`dualBackendScan.e2e.test.ts` live HTTP；renderer 4 例 | 未把冻结列表接入刮削批次 start |
| M12 迁移语义 | 部分 | S12 转换 + S13 首次真实扫描保留无资源成员；两端 HTTP：未映射 STRM 共享 locator 时 preview 含 `strmConflicts`，start 400，目标仍空；映射 STRM 的 `strm_source_path` 与本地 locator 重写到目标挂载，URL locator 不变 | Electron-as-Node `catalogMigrationAcceptance`；`migrationHosts.e2e.test.ts` 5 例 | 跨平台 win32 源路径重写未再单列 |
| M13 启用取消竞争 | 部分 | 两进程并发 enable/abandon；启用后两端重启；丢弃 enable/abandon 响应后再重启；256k tmpfs `ENOSPC` / `chmod 000` `EACCES` 后 enable 回滚 `ready`；Docker 两端成功 enable 后另起第三生产容器对同一包并发 enable/abandon，终态互斥；第四/第五生产容器对同一 400KiB 封面在 `chmod 000` 与 256k tmpfs 后 enable 回滚 `ready`，恢复权限/空间后重试成功 | `migrationHosts.e2e.test.ts`；`catalogMigration.test.ts`；`scripts/server-migration-smoke.mjs` | Windows/macOS 安装包未做 |
| M14 图片与恢复 | 部分 | 源端导出自动解密到包（源正式图不变）；缺别名 start 失败并解冻；正式封面随迁；孤立 uploads 不进包且恢复删除；拷图 `EACCES`/`ENOSPC` 后 enable 回滚；Docker 生产容器重复该回滚并重试 | Electron-as-Node `catalogMigration.test.ts`；`scripts/server-migration-smoke.mjs` | 目标仍拒绝包内残留密文 |
| M15 播放 | 部分 | S11 HTTP Range + S13 真实 mpv：断流后续播、过期 404、seek 2.2、MKV、handoff 后 404；本轮非环回 `10.67.67.1`（eth0 别名）`HEAD`+mpv 解码 | 本机 `/usr/bin/mpv` 0.37.0；ffmpeg 6.1.1；`sudo ip addr add` | WAN 丢包未做；内核无 dummy 网卡 |
...
| D03 工作存储升级 | 部分 | 复制已写入但未 `markReady` 时远程拒绝补开原库；OS `SIGKILL` 落在 `beginCopy` 后、`copyAgentWorkTables` 前；本轮另在打开的 `INSERT OR REPLACE` 事务内 SIGKILL，源行保留、dest 未提交 | `createDesktopRuntime.test.ts`；`createDesktopRuntime.isolation.test.ts` | 不是完整桌面 GUI 升级向导 |
| D04 迟到响应 | 部分 | 既有 generation/`tasks.get` 门闩；live `videos.get` 迟到详情不应用；reconnect 后迟到 `readImage` 不落盘；本轮用户态反代丢掉响应后对客户端 `resetAndDestroy()`，in-flight 查询失败且 generation 前进后再取成功 | 远程后端 + `catalogTaskProgress.test.ts` + `dualBackendScan.e2e.test.ts` + `remoteImageDiskCache.integration.test.ts` + `runtime.test.ts` | WAN 丢包/跨网 RST 未做 |
| D05 取消与退出 | 部分 | 既有 dispose/窗口 rebind；已受理扫描后取消等待、dispose 再建远程后端，原 `operationId` 可查明且任务继续；原生 `BrowserWindow` 在远程扫描 `afterEnumerate` stall 期间关闭再创建，等待中止后仍能按原号查明，放开 stall 后任务终态成功 | Electron-as-Node bootstrap + `mainWindowBindings*.test.ts` + `dualBackendScan.e2e.test.ts`（现 5 例，含 D05 关窗）+ `scripts/test-d05-task-window-close.cjs` | 不是 React 扫描按钮/工具栏关窗产品流 |
