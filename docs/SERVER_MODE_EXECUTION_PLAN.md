...
| M03 编辑冲突 | 部分 | S05/S08 `expectedVersions` 冲突拒绝与回执；评分不 bump V，评分后用评分前 V 改标题成功 | Linux Node 22.14 in-process 宿主 `runtime.test.ts` | 扫描/NFO 改同一目标的交叉未做；是否让评分参与 V 待用户决定 |
