...
- 验证（Linux Node 22.14 / amd64 glibc；另见 `3c9f97b` / `5589d42` / `83d6cf2` / `65c3e1c` / `5ebff2d` / `4822fb6` / `1bac323` / `4c79d67` / `e187266` / `6ad4b84` / `156c1a8` / `5699eae` / `8d3d8f4` / `544e099` / `747ab75` / `933797f` / `e9ee433` / `5401689` / `3152bf1` / `86a82b5` / `870ddf2` / `bc019ee` / `2dd1e54` / `8fc3d9a` / `cb13f72` / `6be92a7`）：
  ...
  - 本会话 `bc-59bde151-be0b-4c9e-8ca2-12376b967412`：`tsc --noEmit` `tsconfig.server.json` 通过。`dualBackendScan.e2e.test.ts` **3 通过**（含 HTTP 宿主 `videos.edit` SIGKILL，以及原生 `BrowserWindow` 在远程扫描 stall 期间关闭再创建）。`migrationHosts.e2e.test.ts` **5 通过**。`sg docker npm run server:smoke:migration` **EXIT 0**（含拷图 `EACCES`/`ENOSPC` 回滚）。未重跑全量 Electron 或 Windows/macOS 安装包。
...
- 未做：Windows NSIS/ZIP 与 macOS DMG 安装包烟测（本 Linux VM 不能诚实构建/签署）；远程 FILE_RENAME（C1）；D04 WAN 丢包后再重连；D05 渲染器扫描按钮/工具栏关窗产品流；M07 三目标分页 UI 拼接；M05 整机断电/fsync 失败；M15 WAN 丢包；完整 M01–M15 / D01–D07。不得把 mock 当完成证据。不得宣称 S13/S14 或 M/D 矩阵已全部完成。
