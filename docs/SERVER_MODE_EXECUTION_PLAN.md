...
- D05：`createWindow` 不注册 IPC；远程 runtime dispose 后再装配仍不打开 `library.db`。主窗口重建调用已注册的 `bindMainWindow` binders，不二次注册 IPC（`870ddf2`）。`6be92a7` 用原生 Electron 关闭后再建 `BrowserWindow`：新 `webContents` 只绑定一次，`ipcMain.handle` 次数不变。本轮：已受理 `scans.run` 后中止 `waitForCatalogTask`，dispose 远程后端再新建连接，`operations.get` 仍能按原 `operationId` 读到 `acceptedTask`/`applied`，`tasks.get` 终态仍是 succeeded/needsInspection。不是任务进行中的完整 GUI 关窗产品流。
...
- 验证（Linux Node 22.14 / amd64 glibc；另见 `3c9f97b` / `5589d42` / `83d6cf2` / `65c3e1c` / `5ebff2d` / `4822fb6` / `1bac323` / `4c79d67` / `e187266` / `6ad4b84` / `156c1a8` / `5699eae` / `8d3d8f4` / `544e099` / `747ab75` / `933797f` / `e9ee433` / `5401689` / `3152bf1` / `86a82b5` / `870ddf2` / `bc019ee` / `2dd1e54` / `8fc3d9a` / `cb13f72` / `6be92a7`）：
  ...
  - 本会话 `bc-59bde151-be0b-4c9e-8ca2-12376b967412`：`tsc --noEmit` `tsconfig.server.json` / `tsconfig.node.json` 通过。先前 `runtime.test.ts` **21 通过**（S09 / 首个 M08 umount / M15 mpv）。续：`catalogOperations.crash.test.ts` **1 通过**（M05 提交前 SIGKILL）。定向 `runtime.test.ts` M06 in-flight claim、M06 扫描 `waitingMaintenance`、M08 页间真实 umount、M09 过期/重启 NFO、D04 TCP RST **5 通过**。随后全量 `apps/server/src/runtime.test.ts` **26 通过 / 0 失败**。本轮续：`createDesktopRuntime.isolation` **6 通过**（含 D03 复制 SQL 事务 SIGKILL）；`migrationHosts.e2e.test.ts` **5 通过**（含未映射 STRM 冲突与映射 STRM 路径重写 HTTP）；`dualBackendScan.e2e.test.ts` **2 通过**（含 HTTP 宿主 `videos.edit` SIGKILL）；全量 `runtime.test.ts` **27 通过 / 0 失败**（含非环回 `10.67.67.1` mpv）。snapshot 无 `docker` 二进制；按 `.cursor/Dockerfile` 现装 Docker CE 28.5.2 / fuse-overlayfs 后 `sg docker npm run server:smoke` **EXIT 0**，`npm run server:smoke:migration` **EXIT 0**（含第三容器 enable/abandon 竞态，以及第四/第五容器拷图 `EACCES`/`ENOSPC` 回滚后重试）。未重跑全量 Electron 或 Windows/macOS 安装包。
...
- 未做：Windows NSIS/ZIP 与 macOS DMG 安装包烟测（本 Linux VM 不能诚实构建/签署）；远程 FILE_RENAME（C1）；D04 WAN 丢包后再重连；D05 任务进行中的完整 GUI 关窗产品流；M07 三目标分页 UI 拼接；M05 整机断电/fsync 失败；M15 WAN 丢包；完整 M01–M15 / D01–D07。不得把 mock 当完成证据。不得宣称 S13/S14 或 M/D 矩阵已全部完成。
...
| M05 受理边界 | 部分 | S06 上传/apply/重启；S09 任务入队；`bebde60` 在打开的 SQLite 事务内 SIGKILL library 子进程；`01266e4` 对独立 Node HTTP 宿主同样在 commit 前 SIGKILL，`operations.get` unknown 后同号重试才应用 | Linux Node 22.14；独立 HTTP 宿主进程 | 不是整机断电/fsync 失败 |
...
| D05 取消与退出 | 部分 | 既有 dispose/窗口 rebind；本轮已受理扫描后取消等待、dispose 再建远程后端，原 `operationId` 可查明且任务继续 | Electron-as-Node bootstrap + `mainWindowBindings*.test.ts` + `dualBackendScan.e2e.test.ts` | 任务进行中的完整 GUI 关窗产品流未做 |
