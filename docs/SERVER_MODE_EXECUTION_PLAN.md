...
- 未做：Docker 两端状态与文件检查；同版本桌面安装包烟测；enable 拷图失败整笔回滚（产品待决）；远程 FILE_RENAME（冻结 IPC 无 resourceId）；D04 物理掐断 TCP/WAN 后再重连；D05 任务进行中的完整 GUI 关窗产品流。不得把 mock 当完成证据。不得宣称 M01–M15 / D01–D07 已全部完成。
