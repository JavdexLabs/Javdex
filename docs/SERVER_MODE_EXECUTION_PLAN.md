...
- M12：启用后对目标 `library.db` 做真实 `scanCoordinator.run`。未映射孤儿片无本地资源、自动清理关闭时成员保留；把 `remove_resource_less_memberships` 改回 1 再扫才会删成员（`e05c165`）。本轮两个 OS 进程：源上两条未映射 STRM 转同一 `direct` locator，`migration.preview` 带 `strmConflicts`，`migration.start` HTTP 400 `INVALID_INPUT`「迁移预检未通过」，源未冻结、无包文件，空目标 `videos` 仍为 0 且 `migration.status` 为迁移不存在（`b3de99b`）。
...
| M12 迁移语义 | 部分 | S12 转换 + S13 首次真实扫描保留无资源成员；本轮两端 HTTP：未映射 STRM 共享 locator 时 preview 含 `strmConflicts`，start 400，目标仍空 | Electron-as-Node `catalogMigrationAcceptance`；`migrationHosts.e2e.test.ts` 4 例 | 映射 STRM 重写的两端 HTTP 未再单列 |
