...
| `packages/library` | 已移入 Node 路径/根/资源身份工具、catalog SQLite、图片存储、扫描辅助与编排、扫描审计读取、分类查询/维护、演员查询/冲突、标签查询、清单维护、本地根护栏、NFO 编解码/导出资料与 sidecar 票据 | Electron 封面导出、影片维护、演员/分类远程图片导入仍待抽离 |
...
| S02 | 进行中（library 已含 db、图片、扫描编排、扫描审计读取、分类查询/维护、演员查询/冲突、标签查询、清单维护、NFO、维护闸门与路径清理） | schema 16。`getDb()` 单例仍保留。剩余：Electron NFO 封面导出、影片维护、演员/分类远程图片导入仍在 desktop；S02D 未开始 |
