...
- 本轮新增 `scripts/server-migration-smoke.mjs` / `npm run server:smoke:migration` **EXIT 0**（`bc-d991d9d2-a4ef-57ac-86c9-f10d2c228a95`；Engine 28.5.2 / fuse-overlayfs）：两个 `javdex-server:smoke` 容器，源与空目标各用独立 `dataDir` 与 `imagesDir` 卷；源夹具按 `migrationHosts.e2e.test.ts` 写入番号 + 正式封面；两端 `docker exec migrate-auth`；宿主经 manage HTTP 走 preview → source start → PUT 包 → target start → enable。断言两端 `migration.status`（源 `frozen` / 目标 `enabled`）、目标正式图文件、源封面仍在（冻结备份）、目标新 `catalogId`。拷图 `EACCES` 回滚仍以既有单测为准，本烟测不重复。本会话续：`4e5c74d` 在同一包再导入第三生产容器后并发 `enable`/`abandon`，终态互斥（enabled 迟到 abandon 仍 enabled；abandoned 迟到 enable 401 `AUTH_REQUIRED`）。
...
- 未做：Windows NSIS/ZIP 与 macOS DMG 安装包烟测（本 Linux VM 不能诚实构建/签署）；远程 FILE_RENAME（C1）；D04 WAN 丢包后再重连；D05 任务进行中的完整 GUI 关窗产品流；M07 三目标分页 UI 拼接；M05 整机断电/fsync 失败；M13 Docker 内拷图 `EACCES`/`ENOSPC` 回滚；M15 WAN 丢包；完整 M01–M15 / D01–D07。不得把 mock 当完成证据。不得宣称 S13/S14 或 M/D 矩阵已全部完成。
