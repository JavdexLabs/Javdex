...
本轮回归覆盖结构化候选输入校验、桌面暂存路径拒绝、图片交付、丢响应后的同号应用重试及本地草稿流程。Windows 宿主上的类型、静态检查、桌面/服务端构建、服务端测试和独立 Node 安装烟测通过，Linux 专属挂载、mpv 和网络故障项保持跳过；本机 Docker daemon 未启动，容器烟测尚未取得本轮通过证据。
