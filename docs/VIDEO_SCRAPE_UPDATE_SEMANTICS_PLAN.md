...
空字段/缺失筛选的图片判定已由 [ADR-0018](adr/0018-inspect-images-only-on-write.md) 改为只看媒体资源引用或远程链接；探活仅发生在写入。下文若仍写「文件不可读视为空」或「按图片健康过滤批量目标」，以 ADR-0018 为准。

## 目标

让影片刮削的 15 个可选字段在“空字段补齐”“有值覆盖”“覆盖更新”下具有一致、可预测的语义，并让字段预判、资源下载、数据库写入、成功状态、批量日志和测试使用同一套判断。

本次不修改插件返回契约，不新增数据库表或持久化更新模式，也不改变手动评分、影片资源和播放数据。
...
- 封面/样张在“空字段补齐”中只看是否已有媒体资源引用或远程链接；本地文件是否可读只在写入时探活（ADR-0018）。
...
| 封面 | `cover_path` 非空 | URL 已下载为可读本地图片 | 无引用时写入 | 下载成功时替换 | 未返回 URL 时清空；下载失败时保留并警告 |
...
| 样张 | 存在带本地路径或远程链接的样张引用 | 全部 URL 下载成功且图片可读 | 无引用时整体写入 | 全部可用时整体替换 | 未返回 URL 时清空；下载失败时保留并警告 |
...
- 样张只看是否存在带本地路径或远程链接的样张行。
- 来源链接按当前 `sourceName` 查询非空 URL。
- 站点评分按当前 `ratingSourceName` 查询有效均分。
- 标签继续只统计刮削标签。

批量任务的 `missingFields` 目标筛选必须与上述规则一致，全部用 SQL 引用/链接谓词表达；`listVideosForBatchScrape` 与 `countVideosForBatchScrape` 复用同一 WHERE，避免列表数量与实际目标不一致。
...
- 单次提示、批量日志和数据库实际结果一致。
- 不修改未选择字段、手动标签、手动评分、影片资源及其他站点元数据。
- 类型检查、定向测试和全量测试全部通过。
