...
| D05 取消与退出 | 部分 | 既有 dispose/窗口 rebind；已受理扫描后取消等待、dispose 再建远程后端，原 `operationId` 可查明且任务继续；原生 `BrowserWindow` 在远程扫描 `afterEnumerate` stall 期间关闭再创建；`818023e` 渲染器点生产「扫描并导入」按钮（preload `scan:run`，已 stamp L/C/G）后关窗再建，原号可查明，放开 stall 后任务成功 | Electron-as-Node bootstrap + `mainWindowBindings*.test.ts` + `dualBackendScan.e2e.test.ts`（现 6 例，含 D05 关窗与扫描按钮）+ `scripts/test-d05-scan-console-window-close.cjs` | 不是完整 sources 设置页其余 chrome |
