...
`src/main/db/scanAuditSource.ts` 已由五处真实读取路径使用，统一 library/run 选择和 identity/bytes/body 三种投影。精确run与latest非NULL明确分开：有有效summary但缺正文不得退回其他run；兼容store只在DB未初始化时读旧文件。legacy JSON为空、损坏或非终态仍保留原可选中规则，随后由各reader做既有验证，不在selector悄悄过滤历史。
