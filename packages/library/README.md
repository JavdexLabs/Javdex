...
Scanner orchestration, NFO import/export, and catalog application services remain under `apps/desktop/src/main` until later S02 slices. Scan helpers (code parse, duration, inventory, local root file guard) live here and stay Node-only. Image storage talks to the desktop host for paths, encryption, and decode. This package must not depend on Electron, Playwright, desktop modules, or the reserved `http`/`server` workspaces.
