...
function allStagedPaths(
  snapshot: NonNullable<ReturnType<typeof getPendingVideoScrapeResolutionSnapshot>>
): string[] {
...
function importCandidateResources(
  ...
  if (wantsFemale || wantsMale) {
    const avatarResources = candidate.resources.filter((resource) => resource.field === 'actressAvatar')
...
export function discardPendingVideoScrapeRecord(pendingScrapeId: number): {
  ok: boolean
  stagedPaths: string[]
} {
  ...
  if (!deleted) return { ok: false, stagedPaths: [] }
  return { ok: true, stagedPaths: deleted.stagedPaths }
}

export function confirmPendingVideoScrape(
  ...
  if (
    ...
    snapshot.pending.revision !== options.expectedRevision
  ) {
    throw new Error('待确认刮削结果已变化，请刷新后重新确认')
  }
  ...
  const application = mediaAssetStore.coordinateDatabaseChange(() =>
    getDb().transaction(() => {
      ...
      for (const { source, candidate } of chosen) {
        const sourceFields = new Set(source.selectedFields.filter((field) => selectedFields.has(field)))
