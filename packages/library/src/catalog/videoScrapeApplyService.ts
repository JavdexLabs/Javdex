...
} from '@library/catalog/classificationIdentityResolver'
...
export const CLASSIFICATION_FIELDS: VideoClassificationField[] = [
  'maker',
  'publisher',
  'series',
  'director'
...
const CLASSIFICATION_FIELD_SET = new Set<VideoClassificationField>(CLASSIFICATION_FIELDS)

export const videoScrapeApplyService = {
  resolveEffectiveFields: resolveEffectiveVideoScrapeFields,
  resolveBatchTargets: resolveVideoBatchTargets,
  preflightClassifications(
    ...
    return planVideoScrapeResult(
      videoId,
      result,
      null,
      [],
      fields.filter((field): field is VideoClassificationField =>
        CLASSIFICATION_FIELD_SET.has(field as VideoClassificationField)
      ),
      undefined,
      mode,
      undefined,
  ...
  plan: planVideoScrapeResult,
  apply: applyScrapeResult
}
