...
import { projectVideoScrapeResult } from '@library/catalog/videoScrapeFieldProjection'
import type {
  MetadataAssetRef,
...
export class LocalNfoSourceAdapter implements VideoMetadataSource {
  ...
  async collectFromAnchors(
    ...
    for (const anchor of anchors) {
      ...
      // Application and pending confirmation use these references to preserve
      // image cardinality. They are opaque labels, never filesystem authority.
      for (const asset of localAssets.assets) {
        if (asset.kind !== 'managed-root-file') continue
        const reference = `managed-root-file:${asset.field}-${asset.position}:${encodeURIComponent(asset.filename)}`
        if (asset.field === 'cover') result.coverUrl = reference
        else if (asset.field === 'samples') (result.sampleImageUrls ??= []).push(reference)
        else if (result.actresses?.[asset.position]) result.actresses[asset.position].avatarUrl = reference
      }
