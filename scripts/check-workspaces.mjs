...
console.log('library currently owns catalog db, image store, public-image HTTP, scan helpers/orchestration/scheduler, scan-audit read, classification query/maintenance/images, actress query/conflict/gallery/maintenance, tag queries, playlist and media-library maintenance, video maintenance/lifecycle, asset migration, NFO, maintenance gate, path cleanup, scrape confirm/candidate apply, playlist applyImport, named target lists, agent metadata findReady/apply/discard, play grants, and manage-image reads; Electron NFO cover export and desktop collection/deliver remain in desktop. packages/http owns LAN browser HTTP and play Range streams; apps/server hosts the independent Node process. This check does not claim a finished management API or remote desktop backend.')
