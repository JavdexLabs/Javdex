 ...
 try{
  ...
  db.transaction(()=>{
   db.exec(`CREATE TABLE maintenance_benchmark_probe(value INTEGER); INSERT INTO maintenance_benchmark_probe VALUES(0);
    ...
    INSERT INTO actress_name_ownership(normalized_name,actress_id) SELECT normalize_actress_name('Collision-'||id),101 FROM actresses WHERE id<=100;
    INSERT INTO actress_names(actress_id,name,type,source) SELECT 101,'Collision-'||id,'alias','manual' FROM actresses WHERE id<=100;`)
   ...
   for(let id=1;id<=100;id++)insert.run(id,id,`Actor-${id}`,JSON.stringify({aliases:[`Collision-${id}`],summary:'x'.repeat(4096)}))
  ...
  function measure(name:string,read:()=>unknown,check:(value:unknown)=>void){
    ...
    const measured={name,samples,medianMs:[...samples].sort((a,b)=>a-b)[1],jsonBytes}
    results.push(measured);console.log(JSON.stringify(measured))
  }
  console.log('stage: legacy oracle')
  ...
  measure('summary.first50',()=>workflow.pageConflictQueue({}),value=>assert.deepEqual(value,{items:summaries.slice(0,50),total:100,offset:0}))
  const selected=oracle[76]
  measure('summary.anchor76',()=>workflow.pageConflictQueue({anchorName:selected.normalizedName}),value=>assert.deepEqual(value,{items:summaries.slice(50,100),total:100,offset:50}))
  console.log('stage: full selected oracle')
  ...
  const result={pending:100,sharedOwnerAliasCount:100,resultSummaryCharacters:4096,results,platform:process.platform,arch:process.arch,
   sqliteVersion:db.prepare('SELECT sqlite_version() AS version').get(),caveats:['100 consistent pending groups with4KiB result text, after ANALYZE; three warm samples per stage, not p95/cold/Windows/HDD/full IPC.',
