...
try{
 server=await createServer({configFile:false,root,cacheDir:path.join(root,'.vite'),plugins:[react(),{name:'synthetic-media',transform(code,id){if(id===path.join(repo,'src/renderer/src/api.ts'))return code.replace('const url = `media://${normalized}`',"const url = '/image.svg?path=' + encodeURIComponent(normalized)")}}],resolve:{alias:{'@shared':path.join(repo,'src/shared'),react:path.join(repo,'node_modules/react'),'react-dom':path.join(repo,'node_modules/react-dom')}},server:{host:'127.0.0.1',port:0,fs:{allow:[repo,root]}}})
 ...
 for(const viewport of [{width:1000,height:640},{width:1440,height:900}]){
  ...
  const candidates=page.locator('button[aria-label^="CODE-"]'),pager=page.getByRole('navigation',{name:'分类封面分页'})
  ...
  await pager.getByRole('button',{name:'下一页'}).click();await page.getByRole('button',{name:'CODE-61',exact:true}).waitFor()
  await page.evaluate(()=>window.__fail=true);await pager.getByRole('button',{name:'下一页'}).click();await page.getByRole('button',{name:'重试'}).click()
  await page.waitForFunction(()=>document.querySelectorAll('button[aria-label^="CODE-"]').length===5)
  assert.ok((await page.locator('.classification-image-preview img').getAttribute('src')).includes('covers%2F1.jpg'))
  await pager.getByRole('button',{name:'上一页'}).click();await page.getByRole('button',{name:'CODE-61',exact:true}).waitFor()
  await pager.getByRole('button',{name:'上一页'}).click();await page.getByRole('button',{name:'CODE-1',exact:true}).waitFor()
  assert.equal(await page.getByRole('button',{name:'CODE-1',exact:true}).getAttribute('aria-pressed'),'true')
  const geometry=await pager.evaluate(el=>{const panel=document.querySelector('[role=tabpanel][data-active=true]'),save=Array.from(document.querySelectorAll('button')).find(b=>b.textContent==='保存主图');return {pager:el.getBoundingClientRect().toJSON(),panel:panel.getBoundingClientRect().toJSON(),save:save.getBoundingClientRect().toJSON(),viewport:innerHeight,horizontal:document.documentElement.scrollWidth>innerWidth}})
  assert.ok(geometry.pager.top>=geometry.panel.bottom);assert.ok(geometry.save.bottom<=viewport.height);assert.ok(geometry.pager.bottom<=geometry.save.top);assert.equal(geometry.horizontal,false)
  ...
  await page.evaluate(()=>window.__hold=true);await pager.getByRole('button',{name:'下一页'}).click();await page.waitForFunction(()=>typeof window.__release==='function')
  ...
  await page.getByRole('button',{name:'CODE-61',exact:true}).waitFor();assert.equal(await candidates.count(),60);assert.equal(await page.evaluate(()=>window.__calls.length),beforeResume+1);assert.ok((await page.locator('.classification-image-preview img').getAttribute('src')).includes('covers%2F1.jpg'))
