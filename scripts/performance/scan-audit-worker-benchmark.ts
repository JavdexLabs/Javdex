...
it('measures responsiveness during real audit worker initialization and paging',async()=>{
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'javdex-audit-worker-probe-')),bundle=path.join(root,'worker.cjs')
 ...
 try{
  ...
  const snapshot={libraryId:1,runId:'worker-probe',finishedAt:'2026-09-11T00:00:00Z'},limits={sourceBytes:128*1024*1024,indexBytes:256*1024*1024,pageBytes:64*1024}
  ...
  const start=performance.now(),first=await client.readAuditPage(snapshot,{section:'files'},limits),startupAndBuildMs=performance.now()-start
  const callbacksDuringBuild=callbacks
  await new Promise(resolve=>setTimeout(resolve,8));clearInterval(timer);timer=undefined
  assert.equal(first.total,count);assert.deepEqual(first.items.map(item=>item.entry),files.slice(0,100));assert.ok(callbacksDuringBuild>0)
  const samples=[]
  for(let i=0;i<4;i++){const start=performance.now(),page=await client.readAuditPage(snapshot,{section:'files',offset:count-100},limits),elapsedMs=performance.now()-start;assert.deepEqual(page.items.map(item=>item.entry),files.slice(-100));if(i>0)samples.push(elapsedMs)}
  console.log(JSON.stringify({count,startupAndBuildMs,callbacksDuringBuild,maxTimerGapMs:maxGapMs,deepPageSamplesMs:samples,firstPageBytes:Buffer.byteLength(JSON.stringify(first)),notes:['2ms main-thread timer; one real worker startup/build, warm filesystem; 8ms drain. Diagnostic timing, not p95 or universal latency bound.','Warm deep page one warmup+3 samples includes client context/message/copy; assertions excluded. Source oracle retained; no peak memory claim.','No UI/IPC handler integration, mixed fairness, idle expiry timing, forced termination during native SQL, Windows/HDD or cold-disk evidence.']},null,2))
