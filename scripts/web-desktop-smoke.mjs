...
try {
  ...
  await page.getByRole('button', { name: '开启配对（5 分钟）', exact: true }).click()
  ...
  await page.getByRole('button', { name: '批准登录', exact: true }).click()
  ...
  await page.getByText('手机扫码打开访问地址', { exact: true }).click()
  ...
  const tv = devices.devices.find(device => device.name.startsWith('客厅电视'))
  assert.ok(tv)
  await page.evaluate(id => window.api.webAccess.deviceRemove(id), tv.id)
