...
import { AGENT_PLATFORM_SCHEMA_SQL } from './schema'
...
describe('database schema', () => {
  ...
  it('closes only v1 plugin-development runs and denies their outstanding approvals', () => {
    const db = new Database(':memory:')
    try {
      ...
      const insertRun = db.prepare(`
        ...
        ) VALUES (?, ?, 'waiting_user', 'v1', '{}', 'pi', '{}', ?, ?)
      `)
      insertRun.run('plugin-v1', 'plugin-developer', '2026-08-20', '2026-08-20')
      insertRun.run('curator', 'library-curator', '2026-08-20', '2026-08-20')
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES (?, ?, 'call', 'digest', 'approved', X'01', '2026-08-20')
      `).run('approval-plugin', 'plugin-v1')
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES (?, ?, 'call', 'digest', 'approved', X'01', '2026-08-20')
      `).run('approval-curator', 'curator')

      migrateDatabase(db)

      assert.deepEqual(
        db.prepare('SELECT id, status FROM agent_runs ORDER BY id').all(),
        [{ id: 'curator', status: 'waiting_user' }, { id: 'plugin-v1', status: 'closed' }]
      )
      assert.deepEqual(
        db.prepare(`
          SELECT request_id, status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals ORDER BY request_id
        `).all(),
        [
          { request_id: 'approval-curator', status: 'approved', cleared: 0 },
          { request_id: 'approval-plugin', status: 'denied', cleared: 1 }
        ]
      )
      assert.equal(tableExistsForTest(db, 'plugin_dev_site_field_mappings'), false)
    } finally {
      db.close()
    }
  })

  it('closes v2 plugin-development runs when upgrading to the Pi-native v3 workspace', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      const insertRun = db.prepare(`
        INSERT INTO agent_runs (
          id, use_case, status, config_revision, config_snapshot_json, runtime_id,
          product_state_json, created_at, updated_at
        ) VALUES (?, ?, 'waiting_user', 'v2', ?, 'pi', ?, ?, ?)
      `)
      insertRun.run(
        'plugin-v2',
        'plugin-developer',
        JSON.stringify({ definitionId: 'plugin-developer', tools: [{ name: 'plugin_test' }] }),
      ...
      insertRun.run(
        'curator-v1',
        'library-curator',
        JSON.stringify({ definitionId: 'library-curator' }),
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('plugin-v2-approval', 'plugin-v2', 'call', 'digest', 'pending', X'01', '2026-08-20')
      `).run()
      ...
      assert.deepEqual(
        db.prepare('SELECT id, status FROM agent_runs ORDER BY id').all(),
        [{ id: 'curator-v1', status: 'waiting_user' }, { id: 'plugin-v2', status: 'closed' }]
      )
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'plugin-v2-approval'
        `).get(),
  ...
  it('closes incompatible PluginDeveloper runs across the v4 and v5 upgrades only', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      const insertRun = db.prepare(`
        INSERT INTO agent_runs (
          id, use_case, status, config_revision, config_snapshot_json, runtime_id,
          product_state_json, created_at, updated_at
        ) VALUES (?, ?, ?, 'revision', '{}', 'pi', ?, '2026-08-21', '2026-08-21')
      `)
      insertRun.run(
        'plugin-v3',
        'plugin-developer',
        'waiting_user',
        JSON.stringify({ schemaVersion: 3, pendingUserRequest: { requestId: 'old' } })
      )
      insertRun.run(
        'plugin-v4',
        'plugin-developer',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v3-approval', 'plugin-v3', 'call', 'digest', 'approved', X'01', '2026-08-21')
      `).run()
      ...
      assert.deepEqual(
        db.prepare('SELECT id, status FROM agent_runs ORDER BY id').all(),
        [
          { id: 'curator', status: 'waiting_user' },
          { id: 'plugin-v3', status: 'closed' },
          { id: 'plugin-v4', status: 'closed' }
        ]
      )
      const v3State = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v3'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(v3State.status, 'cancelled')
      assert.equal('pendingUserRequest' in v3State, false)
      const v4State = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v4'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(v4State.status, 'cancelled')
      assert.match(String(v4State.summary), /ToolPack v5 dry-run/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v3-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
    } finally {
  ...
  it('closes unfinished ToolPack v5 PluginDeveloper runs for the single dry-run v6 upgrade', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      const insertRun = db.prepare(`
        INSERT INTO agent_runs (
          id, use_case, status, config_revision, config_snapshot_json, runtime_id,
          product_state_json, created_at, updated_at
        ) VALUES (?, ?, ?, 'revision', '{}', 'pi', ?, '2026-08-22', '2026-08-22')
      `)
      insertRun.run(
        'plugin-v5',
        'plugin-developer',
        'waiting_user',
        JSON.stringify({ schemaVersion: 4, pendingUserRequest: { requestId: 'old-v5' } })
      )
      insertRun.run(
        'curator-v1',
        'library-curator',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v5-approval', 'plugin-v5', 'call', 'digest', 'approved', X'01', '2026-08-22')
      `).run()
      ...
      assert.deepEqual(
        ...
        [
          ...
          { id: 'plugin-v5', status: 'closed' }
        ]
      )
      const state = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v5'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(state.status, 'cancelled')
      assert.equal('pendingUserRequest' in state, false)
      assert.match(String(state.summary), /ToolPack v6 单一 dry-run/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v5-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
  ...
  it('closes only unfinished PluginDeveloper v6 runs for the Pi-led v7 upgrade', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      insertRun.run(
        ...
        'plugin-developer',
        'waiting_user',
        JSON.stringify({ schemaVersion: 4, pendingUserRequest: { requestId: 'old-v6' } })
      )
      insertRun.run(
        'curator-v1',
        'library-curator',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v6-approval', 'plugin-v6', 'call', 'digest', 'pending', X'01', '2026-08-22')
      `).run()
      ...
      assert.deepEqual(
        ...
        [
          { id: 'curator-v1', status: 'waiting_user' },
          { id: 'plugin-v6', status: 'closed' }
        ]
      )
      const state = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v6'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(state.status, 'cancelled')
      assert.equal('pendingUserRequest' in state, false)
      assert.match(String(state.summary), /ToolPack v7/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v6-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
  ...
  it('closes only unfinished PluginDeveloper v7 runs for the v8 field-contract upgrade', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      insertRun.run(
        ...
        'plugin-developer',
        'waiting_user',
        JSON.stringify({ schemaVersion: 5, pendingUserRequest: { requestId: 'old-v7' } })
      )
      insertRun.run(
        'plugin-v7-history',
        'plugin-developer',
        'settled',
        JSON.stringify({ schemaVersion: 5, summary: 'historical result' })
      )
      insertRun.run(
        'curator-v1',
        'library-curator',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v7-approval', 'plugin-v7-active', 'call', 'digest', 'approved', X'01', '2026-08-22')
      `).run()
      ...
      assert.deepEqual(
        ...
        [
          { id: 'curator-v1', status: 'waiting_user' },
          { id: 'plugin-v7-active', status: 'closed' },
          { id: 'plugin-v7-history', status: 'settled' }
        ]
      )
      const activeState = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v7-active'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(activeState.status, 'cancelled')
      assert.equal('pendingUserRequest' in activeState, false)
      assert.match(String(activeState.summary), /ToolPack v8 字段契约/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v7-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
  ...
  it('closes only unfinished PluginDeveloper v8 runs for the v9 browser workflow upgrade', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      insertRun.run(
        ...
        'plugin-developer',
        'running',
        JSON.stringify({ schemaVersion: 6, pendingUserRequest: { requestId: 'old-v8' } })
      )
      insertRun.run(
        'plugin-v8-history',
        'plugin-developer',
        'settled',
        JSON.stringify({ schemaVersion: 6, summary: 'historical result' })
      )
      insertRun.run(
        'curator-v1',
        'library-curator',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v8-approval', 'plugin-v8-active', 'call', 'digest', 'approved', X'01', '2026-08-22')
      `).run()
      ...
      assert.deepEqual(
        ...
        [
          { id: 'curator-v1', status: 'waiting_user' },
          { id: 'plugin-v8-active', status: 'closed' },
          { id: 'plugin-v8-history', status: 'settled' }
        ]
      )
      const activeState = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v8-active'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(activeState.status, 'cancelled')
      assert.equal('pendingUserRequest' in activeState, false)
      assert.match(String(activeState.summary), /ToolPack v9 增量浏览器/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v8-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
  ...
  it('drops obsolete plugin-development site mappings without changing Agent runs', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      db.exec(`
        CREATE TABLE plugin_dev_site_field_mappings (
          id TEXT PRIMARY KEY,
          site_host TEXT NOT NULL,
          plugin_kind TEXT NOT NULL,
          normalized_label TEXT NOT NULL,
          link_route_signature TEXT NOT NULL DEFAULT '',
        ...
        INSERT INTO plugin_dev_site_field_mappings VALUES (
          'mapping-1', 'missav.example', 'video', '發行商', '/makers/{value}',
          'publisher', 'run-old', 'request-old', '2026-08-20', '2026-08-20'
        );
        INSERT INTO agent_runs (
          id, use_case, status, config_revision, config_snapshot_json, runtime_id,
          product_state_json, created_at, updated_at
        ) VALUES (
          'plugin-current', 'plugin-developer', 'settled', 'revision', '{}', 'pi',
          '{"summary":"keep"}', '2026-08-22', '2026-08-22'
        );
      `)
      ...
      assert.equal(tableExistsForTest(db, 'plugin_dev_site_field_mappings'), false)
      assert.deepEqual(
        db.prepare('SELECT id, status, product_state_json FROM agent_runs').get(),
        { id: 'plugin-current', status: 'settled', product_state_json: '{"summary":"keep"}' }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
    } finally {
      db.close()
  ...
  it('closes only unfinished PluginDeveloper v9 runs for the v10 browser handoff upgrade', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      insertRun.run(
        ...
        JSON.stringify({
          schemaVersion: 6,
          pendingUserRequest: { requestId: 'old-browser', type: 'browser_challenge' }
        })
      )
      insertRun.run(
        'plugin-v9-history',
        'plugin-developer',
        'settled',
        JSON.stringify({ schemaVersion: 6, summary: 'keep history' })
      )
      insertRun.run(
        'curator-v1',
        'library-curator',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v9-approval', 'plugin-v9-active', 'call', 'digest', 'approved', X'01', '2026-08-22')
      `).run()
      ...
      assert.deepEqual(
        ...
        [
          { id: 'curator-v1', status: 'waiting_user' },
          { id: 'plugin-v9-active', status: 'closed' },
          { id: 'plugin-v9-history', status: 'settled' }
        ]
      )
      const state = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v9-active'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(state.status, 'cancelled')
      assert.equal('pendingUserRequest' in state, false)
      assert.match(String(state.summary), /ToolPack v10 通用浏览器交接协议/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v9-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
  ...
  it('closes only unfinished PluginDeveloper v10 runs for the v11 incremental workflow upgrade', () => {
    const db = new Database(':memory:')
    try {
      db.exec(AGENT_PLATFORM_SCHEMA_SQL)
      ...
      insertRun.run(
        ...
        JSON.stringify({
          schemaVersion: 6,
          pendingUserRequest: { requestId: 'old-v10', type: 'freeform' }
        })
      )
      insertRun.run(
        'plugin-v10-history',
      ...
      db.prepare(`
        INSERT INTO agent_approvals (
          request_id, run_id, call_id, args_digest, status, permit_ciphertext, created_at
        ) VALUES ('v10-approval', 'plugin-v10-active', 'call', 'digest', 'approved', X'01', '2026-08-22')
      `).run()
      ...
      assert.deepEqual(
        ...
        [
          { id: 'curator-v1', status: 'waiting_user' },
          { id: 'plugin-v10-active', status: 'closed' },
          { id: 'plugin-v10-history', status: 'settled' }
        ]
      )
      const state = JSON.parse((db.prepare(
        `SELECT product_state_json FROM agent_runs WHERE id = 'plugin-v10-active'`
      ).get() as { product_state_json: string }).product_state_json) as Record<string, unknown>
      assert.equal(state.status, 'cancelled')
      assert.equal('pendingUserRequest' in state, false)
      assert.match(String(state.summary), /ToolPack v11 多轮增量开发/)
      assert.deepEqual(
        db.prepare(`
          SELECT status, permit_ciphertext IS NULL AS cleared
          FROM agent_approvals WHERE request_id = 'v10-approval'
        `).get(),
        { status: 'denied', cleared: 1 }
      )
      assert.equal(db.pragma('user_version', { simple: true }), CURRENT_SCHEMA_VERSION)
