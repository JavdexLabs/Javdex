...
function migrateToV15(database: Database.Database): void {
  ...
  if (tableExists(database, 'agent_runs')) {
    ...
    database.prepare(`
      ...
      SET status = 'denied', decided_at = COALESCE(decided_at, ?), permit_ciphertext = NULL
      WHERE status IN ('pending', 'approved')
        AND run_id IN (SELECT id FROM agent_runs WHERE use_case = 'plugin-developer')
    `).run(at)
    database.prepare(`
      UPDATE agent_runs
      SET status = 'closed', active_operation_id = NULL,
          closed_at = COALESCE(closed_at, ?), updated_at = ?
      WHERE use_case = 'plugin-developer' AND status <> 'closed'
    `).run(at, at)
  }
}
...
function migrateToV17(database: Database.Database): void {
  ...
  const rows = database.prepare(`
    SELECT id, product_state_json
    FROM agent_runs
    WHERE use_case = 'plugin-developer' AND status <> 'closed'
  `).all() as Array<{ id: string; product_state_json: string }>
  const closeRun = database.prepare(`
    UPDATE agent_runs
    SET status = 'closed', active_operation_id = NULL,
        product_state_json = ?, closed_at = COALESCE(closed_at, ?), updated_at = ?
    WHERE id = ?
  `)
  const denyApprovals = database.prepare(`
    UPDATE agent_approvals
    SET status = 'denied', decided_at = COALESCE(decided_at, ?), permit_ciphertext = NULL
  ...
  for (const row of rows) {
    ...
    try {
      state = JSON.parse(row.product_state_json) as Record<string, unknown>
    } catch {
      continue
    }
    ...
    const upgradedState = {
      ...
      summary: 'PluginDeveloper v3 会话已在浏览器 helper v4 升级时关闭；插件草稿保持不变。'
    }
    closeRun.run(JSON.stringify(upgradedState), at, at, row.id)
    denyApprovals.run(at, row.id)
  }
}

function closeOpenPluginDeveloperRuns(
  database: Database.Database,
  summary: string,
  ...
  const rows = database.prepare(`
    SELECT id, product_state_json
    FROM agent_runs
    WHERE use_case = 'plugin-developer'
      ${unfinishedOnly
        ? "AND status IN ('created', 'running', 'waiting_user', 'recovering')"
        : "AND status <> 'closed'"}
  `).all() as Array<{ id: string; product_state_json: string }>
  const closeRun = database.prepare(`
    UPDATE agent_runs
...
function migrateToV18(database: Database.Database): void {
  closeOpenPluginDeveloperRuns(
    database,
    '旧 PluginDeveloper 会话已在 ToolPack v5 dry-run 升级时关闭；插件草稿保持不变。'
  )
}
...
function migrateToV19(database: Database.Database): void {
  closeOpenPluginDeveloperRuns(
    database,
    'PluginDeveloper v5 会话已在 ToolPack v6 单一 dry-run 流程升级时关闭；插件草稿保持不变。'
  )
}
...
function migrateToV20(database: Database.Database): void {
  closeOpenPluginDeveloperRuns(
    database,
    'PluginDeveloper v6 会话已在 Pi 主导的 ToolPack v7 生产运行架构升级时关闭；插件草稿保持不变。'
  )
}
...
function migrateToV21(database: Database.Database): void {
  ...
  for (const row of rows) {
    ...
    closeRun.run(JSON.stringify({
      ...state,
      status: 'cancelled',
      phase: 'waiting_user',
      pendingUserRequest: undefined,
      summary: 'PluginDeveloper v7 会话已在 ToolPack v8 字段契约升级时关闭；插件草稿保持不变。'
    }), at, at, row.id)
    denyApprovals.run(at, row.id)
  }
}

function migrateToV22(database: Database.Database): void {
  closeOpenPluginDeveloperRuns(
    database,
    'PluginDeveloper v8 会话已在 ToolPack v9 增量浏览器与精准行动规则升级时关闭；插件草稿保持不变。',
    true
  )
}

function migrateToV23(database: Database.Database): void {
  database.exec('DROP TABLE IF EXISTS plugin_dev_site_field_mappings')
}

function migrateToV24(database: Database.Database): void {
  closeOpenPluginDeveloperRuns(
    database,
    'PluginDeveloper v9 会话已在 ToolPack v10 通用浏览器交接协议升级时关闭；插件草稿保持不变。',
    true
  )
}

function migrateToV25(database: Database.Database): void {
  closeOpenPluginDeveloperRuns(
    database,
    'PluginDeveloper v10 会话已在 ToolPack v11 多轮增量开发架构升级时关闭；插件草稿保持不变。',
    true
  )
}
...
const MIGRATIONS: Migration[] = [
  ...
  {
    version: 20,
    migrate: migrateToV20
  },
  {
    version: 21,
    migrate: migrateToV21
  },
  {
    version: 22,
    migrate: migrateToV22
  },
  {
    version: 23,
    migrate: migrateToV23
  },
  {
    version: 24,
    migrate: migrateToV24
  },
  {
    version: 25,
