...
export class AgentMetadataBrowserAdapter {
  ...
  async execute(input: {
    ...
    if (action === 'handoff') {
      ...
      const presentation = await session.lease.presentToUser()
      session.finalUrl = presentation.url
    ...
    if (action === 'open') {
      ...
      try {
        const url = await assertAgentMetadataPublicHttpUrl(rawUrl)
    ...
    try {
      const result = await this.evidence.execute({
        ...
        args,
        run: async () => {
          const observation = await scrapeBrowser.runWithLease(
            session.lease,
            () => session.lease.agentAction(commandResult.command)
          )
          if (observation.url) {
            const resolved = await assertAgentMetadataPublicHttpUrl(observation.url)
            if (normalizedHost(resolved.hostname) !== session.allowedHost) {
              throw new Error('页面跳转到了未授权站点，已终止采集。')
            }
            session.finalUrl = observation.url
          }
      ...
      if (isScrapeBrowserChallengeError(error)) {
        ...
        const handoff: AgentMetadataBrowserHandoff = {
          ...
          url: sanitizeAgentMetadataUrl(error.url || presentation.url),
  ...
  async fetchBuffer(runId: string, rawUrl: string, signal?: AbortSignal): Promise<Buffer> {
    const session = this.require(runId)
    await assertAgentMetadataPublicHttpUrl(rawUrl)
    signal?.throwIfAborted()
    return scrapeBrowser.runWithLease(
      session.lease,
      () => session.lease.fetchBuffer(rawUrl, { referer: 'session' })
    )
  }
