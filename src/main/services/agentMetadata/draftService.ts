...
import { agentMetadataBrowser, type AgentMetadataBrowserAdapter } from './browserAdapter'
...
function reviewToken(review: Omit<AgentMetadataReview, 'token'>): string {
  return createHash('sha256').update(canonical(review)).digest('base64url')
}
...
function sanitizedResourceUrl(raw: string): string {
  const url = new URL(raw)
  url.username = ''
  url.password = ''
  url.hash = ''
  for (const key of [...url.searchParams.keys()]) {
    if (/token|secret|key|auth|signature|credential|policy|expires/iu.test(key)) {
...
function sanitizePayloadResourceUrls(payload: AgentMetadataDraftPayload): void {
  if (payload.kind === 'video') {
    if (payload.result.coverUrl) payload.result.coverUrl = sanitizedResourceUrl(payload.result.coverUrl)
    if (payload.result.sampleImageUrls) {
      payload.result.sampleImageUrls = payload.result.sampleImageUrls.map(sanitizedResourceUrl)
    }
    for (const actress of payload.result.actresses ?? []) {
      if (actress.avatarUrl) actress.avatarUrl = sanitizedResourceUrl(actress.avatarUrl)
    }
    return
  }
  if (payload.result.avatarUrl) payload.result.avatarUrl = sanitizedResourceUrl(payload.result.avatarUrl)
  if (payload.result.galleryImageUrls) {
    payload.result.galleryImageUrls = payload.result.galleryImageUrls.map(sanitizedResourceUrl)
  }
}
...
export class AgentMetadataDraftService {
  ...
  apply(input: AgentMetadataApplyInput): AgentMetadataApplyOutcome {
    ...
    const current = this.buildReview(draft, stored.selection, stored.revision)
    if (current.token !== stored.token) return this.staleOutcome(draft, stored)

    try {
      return draft.target.kind === 'video'
  ...
  private parsePayload(
    ...
    if (!identityMatched) throw new Error('外部页面的演员名称与当前演员身份不一致。')
    validateObservedFields({
      observed: observedFields,
      explicitlyEmpty: explicitlyEmptyFields,
  ...
  private async stageResources(
    ...
    for (const request of requests) {
      ...
      try {
        ...
        pending.push({ ...request, remoteUrl: sanitizedResourceUrl(request.remoteUrl), data })
      } catch (error) {
    ...
    if (draft.target.kind === 'video' && draft.payload.kind === 'video' && selection.kind === 'video') {
      ...
      const value: Omit<Extract<AgentMetadataReview, { kind: 'video' }>, 'token'> = {
        ...
        warnings: [
          ...draft.warnings,
          ...plan.warnings,
          ...(identityConflictVideoId ? [`应用后会与影片 #${identityConflictVideoId} 形成重复业务身份。`] : [])
        ],
        classifications: plan.classifications,
        ...(plan.directorChoice ? { directorChoice: plan.directorChoice } : {}),
        ...(identityConflictVideoId ? { identityConflictVideoId } : {}),
        canApply: !plan.directorChoice && !identityConflictVideoId
      }
      return { ...value, token: reviewToken(value) }
    }
    ...
    const outcome = mediaAssetStore.coordinateDatabaseChange(() =>
      getDb().transaction(() => {
        ...
        const galleryAssets = fields.has('gallery')
          ...
          : []
        let outcome: AgentMetadataApplyOutcome | undefined
        ...
        if (!application.applied) {
          ...
          outcome.warnings.push(...application.warnings)
        }
        if (!outcome) throw new Error('演员元数据应用未生成结果。')
        return outcome
      })()
    )
