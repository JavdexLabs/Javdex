...
const submitSchema = {
  ...
  oneOf: [
    strictObjectSchema({
      ...
      observedFields: { type: 'array', uniqueItems: true, items: { type: 'string', enum: videoFields } },
      explicitlyEmptyFields: { type: 'array', uniqueItems: true, items: { type: 'string', enum: videoFields } },
    ...
    strictObjectSchema({
      ...
      observedFields: { type: 'array', uniqueItems: true, items: { type: 'string', enum: actressFields } },
      explicitlyEmptyFields: { type: 'array', uniqueItems: true, items: { type: 'string', enum: actressFields } },
...
export const AGENT_METADATA_COLLECTOR_TOOL_PACK: ToolPack = {
  ...
  tools: [
    ...
    },
    {
      ...
      label: '提交元数据候选',
      description: '提交一次经过身份核对、可由用户预览的影片或演员元数据候选。此工具不直接修改媒体库。',
      schema: submitSchema,
      capability: 'metadata.submit',
      effect: 'write',
      executionMode: 'sequential',
      timeoutMs: 60_000,
