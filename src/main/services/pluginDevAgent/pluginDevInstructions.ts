...
- 不要用原生 read 打开 artifact 文件。\`omittedInlineSections\` 中的每个名称都保证可从同一 artifact 读取；只在 \`nextActions\` 含 \`read-section\`、且某一个被省略 section 明确卡住编码时，调用 \`browser(action="read-section", artifactRef="...", section="...")\`。如果结果有 \`nextCursor\` 且仍需后续内容，原样带入下一次调用。整份 observation 被省略时先读取 \`section="observation"\` 获取可用分区清单；若返回 \`BROWSER_ARTIFACT_SECTION_NOT_FOUND\`，只使用响应中的 \`validSections\`，并按 \`nextAction\` 先读取目录，不要重试或猜测不存在的 section。不要猜测 artifact 的文件、索引或分片格式。
