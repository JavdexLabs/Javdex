...
const formCss = 'src/renderer/src/components/FormPrimitives.module.css'
const settingsCss = 'src/renderer/src/styles/plugin-development-shell.css'
...
describe('form hint styles', () => {
  ...
  it('routes entity-edit helper copy through EditFormField instead of a bare hint class', () => {
    const metadataModal = readFileSync(
      path.resolve('src/renderer/src/components/EditMetadataModal.tsx'),
      'utf8'
    )
    const organizationPicker = readFileSync(
      path.resolve('src/renderer/src/components/OrganizationPickerField.tsx'),
      'utf8'
    )
