...
async function mount(){Component=(await import('./MergeActressModal')).default;await act(async()=>{renderer=TestRenderer.create(element(1))});await settle()}
...
async function click(label:string){const node=button(label);assert.ok(!node.props.disabled);await act(async()=>node.props.onClick());await settle()}
...
afterEach(async()=>{await act(async()=>renderer?.unmount());renderer=undefined;calls.length=0;merges.length=0;heldPages.length=0;holdPage=false;failPage=false;failMerge=false;heldMerge=undefined;merged=0;cancelled=0})

it('pages40 candidates and preserves selection and name plan across pages and search',async()=>{
 await mount();assert.deepEqual(calls,[{keepId:1,search:'',limit:40,offset:0}]);assert.equal(options().length,40)
 ...
 await click('下一页');assert.equal(options().length,21);assert.equal(button('下一页').props.disabled,true)
...
it('retries page failures without losing the selected merge plan',async()=>{
 ...
 await click('下一页');assert.ok(renderer!.root.findAllByProps({role:'alert'}).length);assert.equal(options().length,0)
 await click('重试');assert.equal(options().length,40);assert.equal(radios()[1].props.checked,true)
})
it('ignores old page responses while a changed search is debouncing',async()=>{
 ...
 await act(async()=>heldPages[0].resolve(page({keepId:1,offset:40})));await settle();assert.equal(options().length,0)
...
it('resets session on keep identity change and ignores old page and merge completion',async()=>{
 ...
 await act(async()=>heldPages[0].resolve({items:[],hasMore:false,offset:0}));await settle();assert.equal(options().length,40)
