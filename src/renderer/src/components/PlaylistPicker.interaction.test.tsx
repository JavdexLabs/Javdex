...
afterEach(async () => { await act(async () => renderer?.unmount()); renderer = undefined; requests.length = 0; fullCalls = 0; added.length = 0; created.length = 0 })
...
it('loads only a server page and replaces sixty rows when advancing', async () => {
  await act(async () => { renderer = TestRenderer.create(<Picker videoIds={[7, 2]} onCancel={() => {}} />) })
  assert.equal(fullCalls, 0)
  assert.equal(requests.length, 1)
  assert.equal(requests[0].query.limit, 60)
  await resolve(0)
  assert.equal(renderer!.root.findAll(node => node.props.className === 'playlist-pick-row').length, 60)
...
it('does not create from a stale page or an off-page exact match, and retains full create input', async () => {
  await act(async () => { renderer=TestRenderer.create(<Picker videoIds={[7]} onCancel={()=>{}}/>) })
  await resolve(0)
  const input=renderer!.root.findByType('input')
  const name='A'.repeat(150)
  await act(async()=>input.props.onChange({target:{value:name}}))
  assert.equal(renderer!.root.findAll(node=>node.props.className==='playlist-pick-row').length,0)
...
it('clears error on retry and ignores an old page after searching',async()=>{
  await act(async()=>{renderer=TestRenderer.create(<Picker videoIds={[1]} onCancel={()=>{}}/>)})
...
it('import destination retains a selected identity across server pages without full options',async()=>{
  ...
  await act(async()=>renderer!.root.findByType(Select).props.onChange({target:{value:'1'}}))
  await click('下一页');await resolve(1,60)
  assert.equal(renderer!.root.findByType(Select).props.value,'1')
  assert.equal(React.Children.toArray(renderer!.root.findByType(Select).props.children).length,62)
})
