...
function fieldOptions(kind: AgentMetadataTarget['kind']) {
  return kind === 'video' ? VIDEO_SCRAPE_FIELD_OPTIONS : ACTRESS_SCRAPE_FIELD_OPTIONS
}

function updateModeOptions(kind: AgentMetadataTarget['kind']) {
  return kind === 'video' ? VIDEO_SCRAPE_UPDATE_MODE_OPTIONS : ACTRESS_SCRAPE_UPDATE_MODE_OPTIONS
}
...
function targetLabel(target: AgentMetadataTarget | null): string {
  ...
  return `${target.kind === 'video' ? '影片' : '演员'} #${target.id}`
}
...
export function AgentMetadataCollectorProvider({ children }: { children: ReactNode }): JSX.Element {
  ...
  const close = useCallback((): void => {
    ...
    if (runId && snapshot && ['collecting', 'preparing', 'waiting_user'].includes(snapshot.phase)) {
      void api.agentMetadata.cancel(runId)
    }
    setVisible(false)
    currentRunId.current = null
    currentDraftId.current = null
  ...
  const changeFields = (field: string, checked: boolean): void => {
    ...
    setReview(null)
    void runPlan({ draft, fields, nextMode: mode, directorId: directorSelectionId })
  }
  ...
  const changeMode = (nextMode: VideoScrapeUpdateMode | ActressScrapeUpdateMode): void => {
    ...
    setMode(nextMode)
    setReview(null)
    void runPlan({ draft, fields: selectedFields, nextMode, directorId: directorSelectionId })
  }
  ...
  const resume = async (): Promise<void> => {
    ...
    try {
      const next = await api.agentMetadata.resume({
        ...
        idempotencyKey: `renderer-resume:${crypto.randomUUID()}`
      })
      setSnapshot(next)
    } catch (resumeError) {
  ...
  const apply = async (): Promise<void> => {
    ...
    try {
      const outcome = await api.agentMetadata.apply({
        ...
        idempotencyKey: `renderer-apply:${crypto.randomUUID()}`
      })
      ...
      toast.show(outcome.status === 'no_op' ? '没有需要写入的变化' : 'Agent 元数据已应用', 'success')
      setVisible(false)
      currentRunId.current = null
      currentDraftId.current = null
      onAppliedRef.current = undefined
  ...
  const images = draft?.resources.filter((resource) =>
    ['cover', 'samples', 'avatar', 'gallery'].includes(resource.field)
  ) ?? []
    ...
    <AgentMetadataCollectorContext.Provider value={contextValue}>
      ...
      {visible ? (
        <Modal
          ...
          actions={draft ? (
            <>
              <Button variant="ghost" disabled={busy} onClick={() => void discard()}>丢弃草稿</Button>
                      ...
                      <section className={styles.assets} aria-label="已暂存图片">
                        {images.map((resource) => (
                          <figure key={`${resource.field}:${resource.position}`}>
                            ...
                            <figcaption>{resource.field} · {resource.position + 1}</figcaption>
                          </figure>
                        ))}
                      </section>
                    ) : null}
                      ...
                      <section className={styles.choice}>
                        ...
                        <SelectControl
                          ...
                          onChange={(event) => {
                            ...
                            if (id && draft) {
                              setReview(null)
                              void runPlan({ draft, fields: selectedFields, nextMode: mode, directorId: id })
                            }
                          }}
                        >
                      ...
                      <section className={styles.impacts} aria-label="字段影响预览">
                        ...
                        <div className={styles.impactTable}>
                          {review.impacts.map((impact, index) => {
                            ...
                            return (
                              <div key={`${impact.field}:${'part' in impact ? impact.part ?? '' : ''}:${index}`} data-action={impact.action}>
                                <span>{label}{'part' in impact && impact.part ? ` · ${impact.part}` : ''}</span>
                                <span title={formatValue(impact.currentValue)}>{formatValue(impact.currentValue)}</span>
                                <span aria-hidden>→</span>
                                <span title={formatValue(impact.nextValue)}>{formatValue(impact.nextValue)}</span>
                                <strong>{impact.action !== 'preserve' ? <Check size={13} aria-hidden /> : null}{actionLabel(impact.action)}</strong>
                              </div>
                            )
                          })}
                        </div>
