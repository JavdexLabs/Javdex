        ...
        <SettingsCard
          ...
          actions={
            <div className={styles.statusRow} aria-live="polite">
              <SettingsStatusPill
                status={storageBusy ? 'running' : usingDefault ? 'muted' : 'info'}
              >
                {storageBusy ? '处理中' : usingDefault ? '默认目录' : '自定义目录'}
              </SettingsStatusPill>
          ...
          <div className={styles.grid} aria-busy={storageBusy}>
            ...
            <section className={`${styles.panel} ${styles.cryptoPanel}`} aria-label="图片加密存储">
              <div className={styles.panelHead}>
                <span className={styles.panelIcon} aria-hidden="true">
                  {encryptionEnabled ? (
                    <ShieldCheck {...UI_ICON_SM} />
                  ) : (
                    <ShieldOff {...UI_ICON_SM} />
                  )}
                </span>
                <div className={styles.panelCopy}>
                  <h4>图片加密</h4>
              ...
              <Button
                size="sm"
                disabled={storageBusy}
                onClick={() => onToggleAssetEncryption(!encryptionEnabled)}
              >
                {encryptionEnabled ? '关闭图片加密…' : '启用图片加密…'}
              </Button>
