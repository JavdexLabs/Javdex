...
import { parseFacetOffset, classificationListQueryHash } from '../listView/listQueryParams'

type Input = SeriesListQuery & ClassificationPageQuery & { role?: OrganizationRole }
...
Object.defineProperty(globalThis, 'window', { configurable: true, value: { api: {
  directors: { page, list: () => { throw Error('Full list forbidden') } },
  series: { page, list: () => { throw Error('Full list forbidden') } },
...
const scroll = {
  scrollTop: 0,
  addEventListener: (_event: string, listener: () => void) => { scrollListener = listener },
  removeEventListener: () => { scrollListener = undefined },
...
async function mount(kind: string, query = ''): Promise<void> {
  ...
  await act(async () => {
    renderer = TestRenderer.create(<QueryClientProvider client={client}>
      ...
      element.props.className === 'scroll-body scroll-body--scroll' ? scroll : null })
  })
...
for (const kind of ['series', 'maker', 'director']) describe(`${kind} real list page`, () => {
  it('renders 60/60/5 with server total and bounded page cache', async () => {
    ...
    assert.equal(button('上一页').props.disabled, true)
    await click('下一页'); await wait(() => cards().length === 60 && text(cards()[0]).includes(':61'))
    assert.equal(location.search, '?facetOffset=60')
    await click('下一页'); await wait(() => cards().length === 5)
    assert.equal(button('下一页').props.disabled, true)
    await click('上一页'); await wait(() => cards().length === 60)
    assert.deepEqual(requests.map(query => query.offset), [0, 60, 120, 60])
    assert.ok(requests.every(query => query.limit === 60))
    total = 6000
    for (let offset = 120; offset < 6000; offset += 60) {
      await go(`/facet/${kind}?facetOffset=${offset}`)
      await wait(() => cards().length === 60 && text(cards()[0]).includes(`:${offset + 1}`))
    }
    await wait(() => client.getQueryCache().getAll().length === 1)
  })
  it('clears old cards while loading and supports failed-page retry', async () => {
    ...
    await click('重试'); await wait(() => cards().length === 60)
    assert.ok(text(cards()[0]).includes(':61'))
  })
  it('clamps a shrinking total to its last page, including zero', async () => {
    ...
    await wait(() => location.search === '?facetOffset=60' && cards().length === 1)
    total = 0
    await act(async () => { await client.refetchQueries({ type: 'active' }) })
    await wait(() => location.search === '' && button('下一页').props.disabled && text(renderer.root).includes('共 0'))
    assert.equal(cards().length, 0)
  ...
  it('resets search/sort to page one and rejects late search data', async () => {
    ...
    await wait(() => cards().length === 60 && text(cards()[0]).includes(':new:'))
    await act(async () => resolveOld(result({ search: 'old', offset: 0 })))
    assert.ok(text(cards()[0]).includes(':new:'))
    await click('下一页'); await wait(() => location.search.includes('facetOffset=60'))
  ...
  it('retains page, active observer and scroll through nested detail and return', async () => {
    ...
    await act(async () => { scroll.scrollTop = 432; scrollListener?.() })
    await act(async () => cards()[0].props.onClick())
    ...
    assert.equal(client.getQueryCache().getAll().filter(query => query.isActive()).length, 1)
    await go(-1); await go(-1)
    await wait(() => requests.length >= 2)
    assert.equal(location.pathname, `/facet/${kind}`)
    assert.equal(location.search, '?q=kept&facetOffset=60')
    assert.equal(scroll.scrollTop, 432)
    assert.equal(cards().length, 60)
    assert.ok(requests.every(query => query.offset === 60 && query.search === 'kept'))
  })
})
it('organization role change discards old filters and pending search', async () => {
  ...
  await wait(() => cards().length === 60 && text(cards()[0]).startsWith('publisher::1'))
  await act(async () => { await new Promise(resolve => setTimeout(resolve, 300)) })
...
it('normalizes URL offsets safely and keys distinct pages', async () => {
  for (const raw of [null, '', '-60', 'NaN', 'Infinity', '1e3', '60.5', '9007199254740992']) assert.equal(parseFacetOffset(raw), 0)
  ...
  assert.equal(location.search, '?facetOffset=60')
  assert.ok(requests.every(query => query.offset === 60))
})
