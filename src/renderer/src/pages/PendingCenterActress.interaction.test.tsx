...
it('pages owner candidates and preserves a selected actor outside the current page',async()=>{
 ...
 await act(async()=>vm().dialogs.otherOwner.nextPage());await settle()
 assert.equal(vm().dialogs.otherOwner.options[0].id,2041)
 ...
 assert.equal(vm().dialogs.otherOwner.offset,0);assert.equal(vm().dialogs.otherOwner.options.length,40)
 assert.equal(vm().dialogs.otherOwner.selected?.id,2041)
 await act(async()=>vm().dialogs.otherOwner.nextPage());await settle()
 assert.equal(vm().dialogs.otherOwner.selected?.id,2041)
 await act(async()=>vm().dialogs.otherOwner.nextPage());await settle()
 assert.equal(vm().dialogs.otherOwner.options.length,21);assert.equal(vm().dialogs.otherOwner.hasMore,false)
})
...
it('retries picker failures and invalidates old page replies immediately while search debounces',async()=>{
 ...
 holdPicker=true;await act(async()=>vm().dialogs.otherOwner.nextPage());await settle()
 ...
 assert.equal(vm().dialogs.otherOwner.options.length,0);assert.equal(vm().dialogs.otherOwner.offset,0)
 await act(async()=>{await new Promise(resolve=>setTimeout(resolve,280))});await settle()
