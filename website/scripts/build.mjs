...
async function fetchLatestRelease() {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Latest release is missing required downloads: ${missing.join(", ")}`,
    );
  }
...
async function copyWebsite() {
  ...
  await mkdir(path.join(outputDir, "assets", "screenshots"), {
    recursive: true,
  });
  ...
  await cp(
    path.join(websiteDir, "assets", "icon.png"),
    path.join(outputDir, "assets", "icon.png"),
  );
  await cp(
    path.join(websiteDir, "assets", "LUCIDE-LICENSE.txt"),
    path.join(outputDir, "assets", "LUCIDE-LICENSE.txt"),
  );
  await cp(
    path.join(websiteDir, "assets", "screenshots"),
    path.join(outputDir, "assets", "screenshots"),
    { recursive: true },
  );
...
async function validateOutput(manifest) {
  ...
  const requiredReferences = [
    ...
    "./app.js",
    "./assets/icon.png",
    "./assets/screenshots/library.webp",
...
async function main() {
  ...
  console.log(
    `GitHub Pages site built at ${path.relative(rootDir, outputDir)}/`,
  );
  console.log(
    `Release manifest: ${manifest.version} (${Object.keys(manifest.downloads).length} assets)`,
  );
}
